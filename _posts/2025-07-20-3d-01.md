...
$$
q = (w, a x, b y, c z) \\
l = \sqrt{w^2 + a^2 + b^2 + c^2} \\
$$


$$
q_n = (\frac {w}{l},\frac {a}{l}x,\frac {b}{l}y,\frac {c}{l}z)
$$
...
$$
q = quaternion(w, ax, by,cz) \Rightarrow \begin{matrix}w=cos(\frac {angle}{2})\\ a=sin(\frac {angle}{2}) \cdot axis.x\\ b=sin(\frac {angle}{2}) \cdot axis.y\\ c=sin(\frac {angle}{2}) \cdot axis.z \end{matrix}
$$
...
以上 $q$ 为带旋转的四元数。
  ...
  - 输入四元数 $ v = (0, ix_1, jy_1, kz_1)$ 是空间向量的四元数形式即纯四元数，而输入向量为 3D 空间点 $(x_1,y_1,z_1) $。 
  - 输出四元数 $ \hat {v} = (0, ix_2, jy_2, kz_2) $，而输出向量为 3D 空间点 $(x_2 2,y_2,z_2) $ 即删除之前是加上的零。 
