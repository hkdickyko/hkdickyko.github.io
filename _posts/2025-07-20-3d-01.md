...
先将特定的空间点转移到点 $[0,0,0]$，假设最初的点为 $ x_t, y_t, z_t $。再 **Z** 轴旋转 $\psi$ 度。然后回到原来为位置。即相等于这件物件在指定位置 $ x_t, y_t, z_t $ **Z** 轴旋转 $\psi$ 度。
...
根据以上形式。可发现。先计算的部分比较接近要计算的点坐标。计算公式是由右至左叠加。即计算公式是<font color="#FF1000">由右向左作公式顺序</font>。
