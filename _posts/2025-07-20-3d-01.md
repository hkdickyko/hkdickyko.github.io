...
互联网上的 [四元数计算器](https://www.allmath.com/quaternion-calculator.php)
...
互联网上的 [矩阵数计算器](https://matrixcalc.org)
...
R ＝ 
\begin{bmatrix} 
1-2s\cdot(y^2+z^2) & 2s\cdot(xy-wz) & 2s\cdot(xz+wy) & 0\\
2s\cdot(xy+wz) & 1-2s\cdot(x^2+z^2) & 2s\cdot(yz-wx) & 0\\
2s\cdot(xz-wy) & 2s\cdot(yz+wx) & 1-2s\cdot(x^2+y^2) &0\\
0 & 0 & 0 & 1
\end{bmatrix}
$$

注：$s=\frac{1}{q^2}$，如 $q$ 是归一化的四元数那 $s=\frac{1}{1^2}=1$。 即相等于公式 **28**。用以上方法可以先计算出一個组合矩阵。再用这矩阵直接乘以相关的点，即可得到最终结果如下。


$$
\begin{bmatrix} x_1 \\ y_1 \\ z_1 \\ 1 \end{bmatrix} = (T \cdot R \cdot -
T) \cdot \begin{bmatrix} x_0 \\ y_0 \\ z_0 \\ 1 \end{bmatrix}
$$

以上公式中 $-T$ 是将四元数的参考点转为 $(0, 0, 0)$ 然后旋转至特定的点，最后再将四元数参考点復原至最初位置。例如开始时四元数的参考点原点为 $T=(10, 20, 30)$ 即 $-T=(-10, -20, -30)$ 请参考公式 **30**。
