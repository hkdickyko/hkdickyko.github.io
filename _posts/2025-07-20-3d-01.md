...
互联网上的 [四元数算器] (https://www.allmath.com/quaternion-calculator.php)
 ...
 2. 计算旋转角度：
     - 使用两个向量的点积计算角度的余弦：$cos_{angle} = dot(v1, v2)$。
     - 如果您有一个函数可以直接计算两个归一化向量之间的角度，那就更好了：$angle = angleBetween(v1, v2)$。
 3. 构造四元数：
     - 四元数可以使用公式 $q = (cos(\frac {angle}{2}), sin(\frac {angle}{2}) * axis)$ 构造。

在代码中，这可能看起来像 $q = quaternion(cos(\frac {angle}{2}), sin((\frac {angle}{2}) * axis.x, sin(\frac {angle}{2}) * axis.y, sin(\frac {angle}{2}) * axis.z)$。
