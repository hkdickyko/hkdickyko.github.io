...
$$
q = quaternion(\begin{bmatrix}cos(\frac {angle}{2})\\ sin(\frac {angle}{2}) \cdot axis.x\\ sin(\frac {angle}{2}) \cdot axis.y\\ sin(\frac {angle}{2}) \cdot axis.z \end{bmatrix})
$$
...
假设有两个向量：$ v1 = (1, 0, 0), v2 = (0, 1, 0) $
 ...
 - 角度：$ dot(v1, v2) = 0, cos(angle) = 0, angle = 90°$ 或 $\pi$ 弧度
 - 四元数：$q = (cos(45°), 0, 0, sin(45°))$ 或 $q = (0.707, 0, 0, 0.707)$ 当使用此四元数旋转一个点时，该点将从 $v1$ 方向旋转到 $v2$ 方向。
