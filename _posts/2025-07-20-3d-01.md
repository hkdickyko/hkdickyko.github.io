...
在代码中，这可能看起来像 $q = quaternion(cos(\frac {angle}{2}), sin((\frac {angle}{2}) \cdot axis.x, sin(\frac {angle}{2}) \cdot axis.y, sin(\frac {angle}{2}) \cdot axis.z)$。

示例（概念）：假设有两个向量：$ v1 = (1, 0, 0), v2 = (0, 1, 0) $
 - 轴：$axis = cross(v1, v2) = (0, 0, 1)$（归一化向量）
 - 角度：$ dot(v1, v2) = 0, cos(angle) = 0, angle = 90 \degree$ 或 $\frac{\pi}{2}$ 弧度）
