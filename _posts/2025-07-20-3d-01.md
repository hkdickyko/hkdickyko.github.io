...
因 *Lerp* 插值出来的 $q_t$ 因不是单位四元数，所以要将 $q_t$ 除以它的模 $q_t$ 。  将其转化为单位四元数。 即正规化线性插值（Normalized Linear Interpolation)，NLerp 的输入也一定要是单位向量，否则插值出来的结果不会经过初始和最终向量，其公式如下：
...
$$ Slerp_{(p,q,t)} = q_t = \frac {sin[(1-t)\theta]p+sin[t\theta]q}{sin\theta} $$
 ...
 - 当 $\theta$ 太小时，可以用四元数<font color="#FF1000">正规化线性插值</font>
