...
这个以字叫 RMT 描述符号。一个符号字只描述**一个周期的脉冲**。<font color="#FF1000">发送多少個不同组合的脉冲周期，就形成不同的讯息组合</font>。
...
#define TICK_FREQ 20000 // 频率 20kHz
...
$$
每周期秒 = \frac {1000000}{20000} = 500 \mu s
$$

因为步進电机在 500 PPS 至2000 PPS 即 20000 us - 500 us 之间，所以把 tick 精确设在 500 us 已足够控制步進电机，即 **4** 至 **1** tick之间。
...
```
 ...
 - oSymbols 表示发送 0 时的电平，表示发送 1 时的电平

以下是电平信息的初始化代码：

```c
void init_symbols(int tick)
{
double cycle_us = TICK_FREQ / 1000000;
  symbol.duration0 = tick * cycle_us;
  symbol.level0 = 0;
  symbol.duration1 = tick * cycle_us;
  symbol.level1 = 1;
}
```
...
```c
static rmt_symbol_word_t *oSymbols;

void set_stepper(int index)
{
  if (index > 0)
  {
    oSymbols = malloc(index * sizeof(rmt_symbol_word_t));
...
index 是脉冲组合內的脉冲总数，可以是<font color="#FF1000">不同脉冲长短的组合以达成不同的控制程序</font>。

用以下 ESP 函数，决定有多少位元可以给 **malloc** 使用。
 - ESP.getHeapSize();
 - ESP.getFreeHeap();
 - ESP.getPsramSize();
 - ESP.getFreePsram();
...
void send_data()
{
  ...
  rmt_transmit(txChannel, rfEncoder, oSymbols, sizeof(oSymbols), &cfg));
...
void app_main(void)
{
  init_tx_channel();  // 1、初始化通道
  init_symbols();     // 2、初始化符号
  init_encoder();     // 3、初始化编码器
  while (1)             // 4、使能通道
  {
    set_stepper(1000);
    send_data();
