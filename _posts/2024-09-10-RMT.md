...
static rmt_symbol_word_t sybmol;
```

 - sybmol 表示发送 0 时的电平，表示发送 1 时的电平
...
double cycle_us = TICK_FREQ / 1000000;
  sybmol.duration0 = tick * cycle_us;
  sybmol.level0 = 0;
  sybmol.duration1 = tick * cycle_us;
  sybmol.level1 = 1;
}
```
...
```c
int index = 1;
static rmt_symbol_word_t oSymbols[index];
...
index 是脉冲组合內的脉冲总数，可以是不同脉冲长短的组合以达成不同的控制程序。
...
调用 API 时，可以嵌套在 **ESP_ERROR_CHECK** 宏中，它会自动检查调用是否成功，不成功就输出错误。但为了
