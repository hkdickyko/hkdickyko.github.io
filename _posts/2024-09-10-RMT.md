...
```c
int pts; // |e_hz-s_hz| >= pts

rmt_symbol_word_t* fit(int s_hz, int e_hz, int total)
{
  int tick;
  double freq;
  double steps = (e_hz-s_hz)/(total-1);
  rmt_symbol_word_t *curve = rmt_alloc_encoder_mem(total * sizeof(rmt_symbol_word_t));
  for (int i=0; i<total; i++) 
  {
    freq = smoother(s_hz, e_hz, s_hz+steps*i);
    tick = TICK_FREQ/freq/2;
    curve[i].level0 = 1;
    curve[i].duration0 = tick;
    curve[i].level1 = 0;
    curve[i].duration1 = tick;
  }
  return curve;
}
```
...
```c
ESP.getHeapSize() // Heap 总大小
ESP.getFreeHeap() // 可用剩余空间大小
uxTaskGetStackHighWaterMark(taskHandle) // 计算当前任务剩余多少内存
```

 - malloc()函数：在用malloc开辟空间时，用完一定要释放空间，如果不释放会造成内存泄漏。malloc()函数返回的实际是一个无类型指针，必须在其前面加上指针类型强制转换才可以使用。
...
void init_symbol()  // 初始化符号
{
symbols = fit(sfreq,efreq,total); 
}
