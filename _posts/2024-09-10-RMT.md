...
$$
tick = 需要間隙数量 = \frac {1}{2} \times \frac {计算频率間隙}{分辨频率間隙} =  \frac {1}{2} \times \frac {\frac {1}{y}s} {\frac {1}{x}s} \\ tick =  \frac {1}{2} \times \frac {x}{y}H_z
$$

```c
int pts; // |e_hz-s_hz| >= pts

void fit(int s_hz, int e_hz, int pts)
{
  int tick;
  double freq;
  int steps = (e_hz-s_hz)/(pts-1);
  for (int i=0; i<pts; i++) 
  {
