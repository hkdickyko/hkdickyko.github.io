...
CMake 也可以通過屬性來存儲信息。這就像是一個變量，但它被附加到一些其他的物體 (item) 上，像是一個目錄或者是一個目標。一個全局的屬性可以是一個有用的非緩存的全局變量。許多目標屬性都是被以 CMAKE_ 為前綴的變量來初始化的。例如你設置 CMAKE_CXX_STANDARD 這個變量，這意味著你之後創建的所有目標的 CXX_STANDARD 都將被設為CMAKE_CXX_STANDARD 變量的值。
...
通過名字來傳遞變量需要注意兩點：
 ...
 - 函數調用處用變量的名字 <font color="#FF1000">var</font>，而不是它的值 <font color="#FF1000">${var}</font>
 - 在函數內部，set的時候，要加上作用域 <font color="#FF1000">PARENT_SCOPE</font>
