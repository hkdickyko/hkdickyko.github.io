...
## 三軸加速度計 ( $ \theta, \phi, \varphi $ )

### 繞 X 軸旋轉角度 $ \theta $ = roll	

$$ \theta=\arctan\left(\frac{A_{y}}{A_{z}}\right) $$

### 繞 Y 軸旋轉角度 $ \phi $ = pitch 

$$ \phi=\arctan\left(\frac{-A_{x}}{\sqrt{A_{y}^2+A_{z}^2}}\right) $$ 


## 三軸加速度計和磁力計估計偏航 *yaw*

### 繞 Z 軸旋轉角度 $ \varphi $ = yaw
...
$$ \varphi=\arctan\left(\frac{M_{y}}{M_{x}}\right) $$

 - A<sub>x</sub>, A<sub>y</sub>, A<sub>z</sub> 是 x, y 和 z 軸加速度變化 
 - m<sub>x</sub>, m<sub>y</sub>, m<sub>z</sub> 是 x, y 和 z 軸磁場強度變化
