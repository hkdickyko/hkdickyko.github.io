...
陀螺仪偏差的模型为 $\dot b_g=b_{bg}(t)∼\mathcal{N}(0,Q_g)$，其中 $Q_g$ 是模拟陀螺仪噪声的协方差矩阵。
...
加速度偏差的模型为 $\dot b_a=b_{ba}(t)∼\mathcal{N}(0,Q_a)$，其中 $Q_s$ 是模拟加速度噪声的协方差矩阵。

在这里，传感器的方向可以从外部来源（例如运动捕捉系统或相机）获知，也可以通过传感器融合进行估计。
...
因内容超越了本章介绍目的。详细资料。请参考：[互联网上滤波器详解](https://www.olliw.eu/2013/imu-data-fusing/#chapter23)
...
Mahony 滤波器是一个互补滤波器，它用四元数空间中的流形变换。Mahony 滤波器的总体思想是通过融合/组合姿态估计来估计姿态/角度/方向 $\begin{matrix}{I}\\{W}\end{matrix}q_{t+1}$ ，即通过积分陀螺仪测量值 $\begin{matrix}{I}\\{W}\end{matrix}q_{\omega}$ 和加速度计测量值获得的方向。首先计算上一步中的方向误差，然后应用基于比例积分 (PI) 补偿器的校正步骤来校正测量的角速度。该角速度在四元数流形上传播并积分以获得姿态的估计值。
