...
Mahony 滤波器是一个互补滤波器，它用四元数空间中的流形变换。Mahony 滤波器的总体思想是通过融合/组合姿态估计来估计姿态/角度/方向 $I_wq_{t+1}$ ，即通过积分陀螺仪测量值 $I_wq_{\omega}$ 和加速度计测量值获得的方向。首先计算上一步中的方向误差，然后应用基于比例积分 (PI) 补偿器的校正步骤来校正测量的角速度。该角速度在四元数流形上传播并积分以获得姿态的估计值。
