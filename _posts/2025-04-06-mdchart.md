...
<tr><td>主接口<br>触发</td><td>USER_CTRL<br>0x03<br>I2C_IF_DIS [4]</td><td>写入</td><td>0</td><td>0x03, [0] << 4</td><td class ="left">1 – SPI 模式<br>0 – I2C 模式</td></tr>
...
uint8_t readSlave4(uint8_t addr, uint8_t reg){
  addr = addr | (1 << 7);
  writeBytes(2, REG_BANK_SEL, 3 << 4);
...
uint8_t writeSlave4(uint8_t addr, uint8_t reg, uint8_t content){
  addr = addr & ~(1 << 7);
  writeBytes(2, REG_BANK_SEL, 3 << 4);
  writeByte(I2C_SLV4_CTRL);
  uint8_t value = readByte();
...
uint8_t readSlave0(uint8_t addr, uint8_t reg){
  addr = addr | (1 << 7);
  writeBytes(2, REG_BANK_SEL, 3 << 4);
