...
<tr><td>设置陀螺仪<br>采样率</td><td>GYRO_SMPLRT_DIV<br>0x00 [7:0]</td><td>写入</td><td>2</td><td>0x00, [7:0]</td><td class="left">此寄存器仅在 FCHOICE = 1'b1<br>(FCHOICE_B 寄存器位为 1'b0)<br>且 (0<DLPF_CFG<7) 时有效。<br>ODR (输出数据速率) 计算如下:<br>=$\frac{1100}{1+GYRO\_SMPLRT\_DIV}H_z$
$GYRO\_SMPLRT\_DIV = \frac{1100-ODR}{ODR}$</td></tr>
