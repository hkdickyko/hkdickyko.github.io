...
```cpp
uint8_t readSlave(uint8_t addr, uint8_t reg){
  esp_err_t ret;
  addr = addr | (1 << 7);
  writeBytes(2, 0x7F, 3 << 4);
  writeByte(0x15);
  uint8_t value = readByte();
  if(value && (1 << 7) != 0)
    printf("i2c slave: transaction not idle.\n");
  writeBytes(2, 0x13, addr);
  writeBytes(2, 0x14, reg);
  writeBytes(2, 0x15, 1 << 7);
  delay(5);
  writeByte(0x17); 
  value = readByte();
  writeBytes(2, 0x7F, 0);
  ...
  if((status && (1 << 4)) != 0) {
    printf("Slave not responding (NACK).\n"); 
    return -ENODEV;   
  }
  if((status && (1 << 6)) == 0){
    printf("Transaction not done.\n");
    return -ETIMEDOUT;
  }
  return value;
}
...
```cpp
uint8_t writeSlave(uint8_t addr, uint8_t reg, uint8_t content){
  addr = addr & ~(1 << 7);
  writeBytes(2, 0x7F, 3 << 4);
  writeByte(0x15);
  uint8_t value = readByte();
...
```c
  addr = addr | (1 << 7);
  writeBytes(2, 0x7F, 3 << 4);
  writeBytes(2, 0x03, addr);
  writeBytes(2, 0x04, 0x11);
  writeBytes(2, 0x05, 0x86 | len);
```