...
int main(void)
{
  ...
  for(int k=0; k<N; k++) {
    v[k].Re = 0.13*cos(2*PI*k/(double)N);
    v[k].Im = 0; // 0.125*sin(2*PI*k/(double)N);
    v1[k].Re =  0.3*cos(2*PI*k/(double)N);
    v1[k].Im = 0;// -0.3*sin(2*PI*k/(double)N);
  }
  ...
  putchar('\n');
  print_vector("Orig", v1, N, fs);
  fft(v1, N, scratch);
  print_vector(" FFT", v1, N, fs);
  ifft(v1, N, scratch);
  print_vector("iFFT", v1, N, fs);
}
