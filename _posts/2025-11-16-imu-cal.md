...
以三轴陀螺仪、三轴加速度计、三轴磁力计和气压计做说明，首先要先了解三轴之间的关系，假设有三个向量 $X、Y、Z$，其方向分别为三个轴的方向，那这三个向量会互相垂直、正交（Orthogonality），也就像是 $X \cdot Y = 0$ 或 $X \times Y = Z$。可用右手定则来了解。
...
同样也是将向量从机体坐标系 $b$ 到大地坐标系 $R$ 的姿态矩阵 $C_b^R$（也称为坐标转换矩阵）即为：
...
这里再给出从大地坐标系 $R$ 转换到机体坐标系 $b$ 的坐标转换矩阵 $
C_R^b$
...
\operatorname{atan2}(2(q_w\color{red}{q_x} + q_yq_z),  q_w^2 - q_x^2 - q_y^2 \color{red}{+ q_z^2}) \\
\operatorname{asin}(2(q_w\color{red}{q_y} - q_xq_z)) \\
\operatorname{atan2}(2(q_w\color{red}{q_z} + q_xq_y),  q_w^2 \color{red}{+ q_x^2} - q_y^2 - q_z^2) \\ 
\end{bmatrix}
$$
...
设有大地坐标下的重力加速度 $g$，把 $g$ 通过姿态矩阵（坐标转换矩阵）的逆（意味着从地理坐标系 $R$ 到机体坐标 $b$ 系）变换到机体坐标系，得到其在机体坐标系下的 **理论重力加速度向量 $\hat{v}$**，则两者的变换关系可通过前文给出的姿态矩阵得出：
...
$$
\hat{g_b} = \begin{bmatrix} \hat{g_{x}} \\ \hat{g_{y}} \\ \hat{g_{z}}\end{bmatrix} = C_R^b \cdot \hat{g_R} = \left ( C_b^R \right)^{-1} \cdot \begin{bmatrix} 0 \\ 0 \\ 1\end{bmatrix} = \begin{bmatrix}
2(q_xq_z + q_wq_y) \\
2(q_yq_z - q_wq_x) \\
...
可以看出，将重力加速度向量变换至机体坐标系后，恰好是矩阵的最后一列。这样就得到了由描述刚体姿态的四元数推导出的理论重力加速度向量 $\hat{v}$。还可以通过加速度计测量出实际重力加速度向量 $\bar{v}$。

显然理论重力加速度向量 $\hat{v}$ 和实际重力加速度向量 $\bar{v}$ 之间必然存在偏差，而这个偏差很大程度上是由陀螺仪数据产生的角速度误差引起，根据理论向量和实际向量间的偏差，就可以补偿陀螺仪数据的误差，进而解算出较为准确的姿态，即将隐含在四元数中的角速度误差显化。

单位四元数的定义即是模为 1 的四元数： 

$$
\because |q|=\sqrt{q_w^2+q_x^2+q_y^2+q_z^2}=1 \\
\therefore \color{green}{-q_x^2-q_y^2} = \color{red}{q_w^2+q_z^2-1} \\
q_w^2\color{green}{-q_x^2-q_y^2}+q_z^2 = q_w^2+\color{red}{q_w^2+q_z^2-1}+q_z^2 \\
 =2(q_w^2+q_z^2-0.5)
$$
...
对测量重力加速度向量 $\hat{g}$ 与理论重力加速度向量 $g$ 做叉积其结果必然为零，因为加速度和重力向量是平行向量。而任意两个平行或反平行向量的 $\times$ 叉积 (Cross product) 始终为零。



$$
a_r \times \hat{g} = \begin{bmatrix} 
\hat {i} & \hat {j} & \hat {k} \\
a_{rx} & a_{ry} & a_{rz} \\ 
...
$$
Output = \left ( K_p \times E_r \right) + \left(K_i \times \int E_r \cdot dt \right) + \left(K_d \times \frac {dE_r}{dt} \right)，E_r : 误差
$$


$$
Output = \left ( K_p \times E_r \right) + \left(K_i \times \int E_r \cdot \Delta t \right) + \left(K_d \times \frac {E_r(t) - E_r(t-1) }{\Delta t} \right)
$$


$$
\begin{bmatrix} 
\bar{e_x} \\ \bar{e_y} \\ \bar{e_z} \end{bmatrix} = K_p \times \begin{bmatrix} e_x \\ e_y \\ e_z \end{bmatrix} + K_i \times \int \begin{bmatrix} e_x \\ e_y \\ e_z \end{bmatrix} \cdot \Delta t + K_d \times \frac {1}{\Delta t} \cdot \left ( {\begin{bmatrix} e_x \\ e_y \\ e_z \end{bmatrix}}_{t} - {\begin{bmatrix} e_x \\ e_y \\ e_z \end{bmatrix}}_{t-1} \right )
$$
...
$$
q_{t+\Delta t} = q_{t} + \frac {\Delta t}{2}\begin{bmatrix}
