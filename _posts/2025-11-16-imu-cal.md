...
$$
\frac{1}{2}\hat{g_x} = q_xq_z - q_wq_y\\
\frac{1}{2}\hat{g_y} = q_yq_z + q_wq_x\\
\frac{1}{2}\hat{g_z} = q_w^2+q_z^2-0.5
$$

$\hat{g_x}、\hat{g_y}、\hat{g_z}$ ，其实是用上一次机体姿态（四元数）换算出来的在当前的机体坐标系上的重力单位向量。注意，代码中取了其中的一半，下面的误差同理。
