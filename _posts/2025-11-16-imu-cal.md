...
q_w^2\color{red}{-q_x^2-q_y^2}+q_z^2 \Leftrightarrow  q_w^2+q_w^2+q_z^2-1+q_z^2 \\
 = 2(q_w^2+q_z^2-0.5)\\
$$


$$
\therefore q_w^2+q_z^2 = \color{green}{-q_x^2-q_y^2 +1}  \\
\color{red}{q_w^2}-q_x^2-q_y^2+\color{red}{q_z^2} \Leftrightarrow -q_x^2-q_y^2 +1 -q_x^2-q_y^2 \\
 = 2(-q_x^2-q_y^2+0.5)\\ 
$$
...
对测量重力加速度向量 $\hat{g}$ 与理论重力加速度向量 $g$ 做叉积其结果必然为零，因为加速度和重力向量是平行向量。而任意两个平行或反平行向量的 $\otimes$ 叉积 (Cross product) 始终为零。
...
$$
h_x = 2(q_w^2+q_x^2-0.5) \cdot m_x + 2(q_xq_y + q_wq_z) \cdot m_y + 2(q_xq_z - q_wq_y) \cdot m_z，\\
h_y = 2(q_xq_y - q_wq_z) \cdot m_x + 2(q_w^2+q_y^2-0.5) \cdot m_y + 2(q_yq_z + q_wq_x) \cdot m_z，\\
h_z = \sqrt(h_x^2 + h_y^2)
$$
