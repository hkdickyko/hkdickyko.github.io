...
$$
\hat{g_b} = \begin{bmatrix} \hat{g_{bx}} \\ \hat{g_{by}} \\ \hat{g_{bz}}\end{bmatrix} = C_R^b \cdot \hat{g_R} = \left ( C_b^R \right)^{-1} \cdot \begin{bmatrix} 0 \\ 0 \\ 1\end{bmatrix} = \begin{bmatrix}
2(q_xq_z + q_wq_y) \\
2(q_yq_z - q_wq_x) \\
...
\therefore -q_x^2-q_y^2 = \color{red}{q_w^2+q_z^2-1} \\
q_w^2-q_x^2-q_y^2+q_z^2 = q_w^2+\color{red}{q_w^2+q_z^2-1}+q_z^2 \\
 =2(q_w^2+q_z^2-0.5)
$$
...
$$
\frac{\hat{g_{bx}}}{2} = q_xq_z - q_wq_y\\
\frac{\hat{g_{by}}}{2} = q_yq_z + q_wq_x\\
\frac{\hat{g_{bz}}}{2} = q_w^2+q_z^2-0.5
$$

$\frac{v_x}{2}、\frac{v_y}{2}、\frac{v_z}{2}$ ，其实是用上一次机体姿态（四元数）换算出来的在当前的机体坐标系上的重力单位向量。注意，代码中取了其中的一半$\frac{1}{2}$，下面的误差同理
