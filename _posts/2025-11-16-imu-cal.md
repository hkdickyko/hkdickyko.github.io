...
四元数 ： $ q = [q_w， q_x，q_y，q_z] $ 在 NED 系统。
...
$$
v = \begin{bmatrix}0， v \end{bmatrix} ， q = \begin{bmatrix} cos\left(\frac{\theta}{2}\right)，sin\left(\frac{\theta}{2}\right) \begin{bmatrix}x \\ y \\ z\end{bmatrix} \end{bmatrix}
$$
...
**地球引力的方向大致指向地心**，在局部尺度上，引力指向质心。设有大地坐标下的重力加速度 $g$，把 $g$ 通过姿态矩阵（坐标转换矩阵）的逆（意味着从地理坐标系 $R$ 到机体坐标 $b$ 系）变换到机体坐标系，得到其在机体坐标系下的 **理论重力加速度向量 $\hat{v}$**，则两者的变换关系可通过前文给出的姿态矩阵得出：
...
$$
\begin{bmatrix} \hat{g_{x}} \\ \hat{g_{y}} \\ \hat{g_{z}}\end{bmatrix} = C_R^b \cdot \hat{g_R} = \left ( C_b^R \right)^{-1} \cdot \begin{bmatrix} 0 \\ 0 \\ 1\end{bmatrix} = \begin{bmatrix}
2(q_xq_z + q_wq_y) \\
2(q_yq_z - q_wq_x) \\
...
可以看出，将重力加速度向量变换至机体坐标系后，恰好是矩阵的最后一列。这样就得到了由描述刚体姿态的四元数推导出的理论重力加速度向量 $\hat{g}$。还可以通过加速度计测量出实际重力加速度向量 $\bar{a}$。

显然理论重力加速度向量 $\hat{a}$ 和实际重力加速度向量 $\bar{g}$ 之间必然存在偏差，而这个偏差很大程度上是由陀螺仪数据产生的角速度误差引起，根据理论向量和实际向量间的偏差，就可以补偿陀螺仪数据的误差，进而解算出较为准确的姿态，即将隐含在四元数中的角速度误差显化。
