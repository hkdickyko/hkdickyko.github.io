...
$$
\begin{bmatrix} roll_x \\ pitch_y \\ yaw_z \\ \end{bmatrix} = \begin{bmatrix} \theta_x \\ \phi_y \\ \psi_z \\ \end{bmatrix} = \begin{bmatrix} 
...
-w_x q_x - w_y q_y - w_zq_z \\
w_x q_w - w_y q_z + w_zq_y \\
w_x q_z + w_y q_w - w_zq_x \\
-w_x q_y + w_y q_x + w_zq_w
\end{bmatrix}
$$
...
对实际重力加速度向量 $\bar{v}$ 与理论重力加速度向量 $g$ 做叉积其结果必然为零，因为加速度和重力向量是平行向量。而任意两个平行或反平行向量的 $\times$ 叉积 (Cross product) 始终为零。
...
因为加速度和重力向量的和必然为零，所以计算后 $e_x、e_y、e_z$ 为计算误差的数值。基于以上计算结果，可以用 PID 数学补偿方法来修正误差值。使计算值尽量接近零，用以已修正测量误差。
