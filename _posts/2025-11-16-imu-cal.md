...
导航坐标系的 $x$ 轴与正北对齐，故可以将磁力计测量数值 $\hat{m_x}、\hat{m_y}$ 的平面的投影折算到世界坐标系 $B_h$ 轴。而 $z$ 轴不改变，公式如下：


$$
h_x = \sqrt{B_x^2 + B_y^2}\\
h_y = 0;\\
h_z = B_z;
$$
...
$$
\hat{h} = C_R^b \cdot \begin{bmatrix}h_x \\ h_y \\ h_z \end{bmatrix} \Rightarrow = C_R^b \cdot \begin{bmatrix}h_x \\ 0 \\ h_z \end{bmatrix}
$$
...
$$
\hat{h_x} = 2 \cdot (0.5 - q_y^2 - q_z^2)\cdot B_h + 2 \cdot (q_xq_z - q_wq_y) \cdot B_z；\\
\hat{h_y} = 2\cdot(q_xq_y - q_wq_z)\cdot B_h + 2 \cdot (q_yq_z + q_wq_x) \cdot B_z;\\
\hat{h_z} = 2 \cdot (q_xq_z + q_wq_y)\cdot B_h + 2 \cdot (0.5 - q_x^2 - q_y^2)\cdot B_z;
$$
