...
```java
public byte[] loadData(InputStream stream)
{
  try {
...
{
  try {
    InputStream stream =getAssets().open("core");
    byte[] bytes = loadData(stream);
    return bytes;
  }
  catch (IOException er)
  {er.printStackTrace();}
...
## 訪問 Android 的 Cache 目錄文件

```java
private String readFile(String fileName)
{
  try {
    String fileName = getCacheDir() + fileName;
    BufferedInputStream bStream = new BufferedInputStream(new FileInputStream(fileName));
    byte[] bytes = loadData(bStream);
...
# 從字節 Zip 數組到 unZip 文件並保存到 Cache 目錄

```java
public void unCryto(byte[] bytes)
{
  try
  {
    ...
    {
      String fileName=cacheDir + "/" + zEntry.getName();
      if (zEntry.isDirectory())
        hanldeDirectory(fileName);
      else
      {
