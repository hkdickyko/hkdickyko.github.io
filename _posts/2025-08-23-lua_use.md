...
file(GLOB LIB_LIST src/*.c)
list(REMOVE_ITEM LIB_LIST src/lua.c src/luac.c)
add_library(liblua SHARED ${LIB_LIST})

#add_executable(lua ${SRC_LIST})
#add_executable(luac ${SRC_LIST})
```

CMakeLists.txt 文件包含一组指令和说明，用于描述项目的源文件和目标。以上 CMakeLists.txt 文件为例。必须有以下部份：

 - **cmake_minimun_required**(VERSION 3.10.0) 开头，指定了处理该项目所需的 CMake 最低版本。在本例中，需要 3.10.0 版本，否则会报错。
 - **project**(main_prog) 指定传递给 cmake 命令的项目名称为 main_prog。
 - **add_executable**(main_prog main.cpp age.cpp) 添加一个名为 main_prog 的可执行目标，该目标由指定的源文件（即 main.cpp 和 age.cpp）构建。
 - **file**(GLOB_RECURSE SRC_LIST *.c) 可不使用，因只是将所有 C 的源文件档案变成一个序列。
