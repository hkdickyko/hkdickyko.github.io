...
|AT+CIICR|OK	|與服務提供商建立無線連接並獲取<font color='#FF0010'>IP</font地址|
|AT+CIFSR|10.63.87.28|顯示獲取連接成功後分配給模塊的<font color='#FF0010'>IP</font地址|
...
[SIM800L 驅動程序的網絡資源](https://github.com/pythings/Drivers)
...
def example_usage():
    ...
    modem.connect(apn='pccw') #leave username and password empty if your network don't require them
    print('\nModem IP address: "{}"'.format(modem.get_ip_addr()))
    # Example GET
    print('\nNow running demo http GET...')
    url = 'http://61.93.172.179:88/'
