...
<font color="#FF1000">法線貼圖</font>是一種模擬物體表面凹凸和皺紋的技術，複雜的三維模型 3D 美術可以通過減面操作把精模簡化為簡模，然後把精模表面的複雜幾何資訊對映到法線貼圖上。可使用法線比例參數 (normalScale) 調整凹凸效果。

<font color="#FF1000">凹凸貼圖</font>和法線貼圖相似。 結果是制作出一個明顯凹凸不平的表面，而不是一個光滑的表面。但底層對象的表面實際上並沒有改變。 無法使用這種技術傾斜相機來查看 3D 山脈。也可以使用凹凸比例參數 (bumpScale) 調整凹凸效果（如地圖對光照的影響程度）。

<font color="#FF1000">注意</font>: 如果模型定義了法線貼圖，就沒有必要在使用凹凸貼圖。


<font color="#FF1000">高光貼圖</font>是用於定義表面光澤度和高亮顏色的貼圖。像素值越高（從黑色到白色），表面就會顯得越亮。 因此如乾石或棉織物之類的表面往往具有非常暗的高光貼圖，而像拋光鉻或塑料這樣的表面往往具有較淺的高光貼圖。
