...
mtlLoader.load("./male02/male02.mtl", (mtl) => {
  mtl.preload();
  gloader.setMaterials(mtl);
  gloader.load("./male02/male02.obj", (root) => {
    scene.add(root);
  });
});
```
...
tloader.load("fonts/helvetiker_regular.typeface.json", function (font) {
  const geometry = new TextGeometry("渲染場景 three.js!", {
    font: font,
    size: 2,
    height: 0.1,
  });
