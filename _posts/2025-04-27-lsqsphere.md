...
伪逆和矩阵逆之间没有“区别”，因为如果一个矩阵有逆，那么它也一定有一个唯一的伪逆，并且与逆相同。只要它们都存在，就必然相同。但许多矩阵没有逆：非方阵，以及行列式为零的方阵（“不可逆”）。所有这些矩阵，甚至包括全零的矩形矩阵，都有伪逆。所以用伪逆作为计算方法。

伪逆 $A^{+}$ 是一个常见用作计算无精确解的线性方程组的“最佳拟合”（最小二乘）近似解。
