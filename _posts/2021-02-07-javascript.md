...
jQuery 簡化了JavaScript 的操作方法。只要引入 jQuery 程式庫。就可以運用「＄」字符號為字首。類似CSS選擇器的DOM操作。更可通過jQuery使用大量外掛程式庫。有效地簡化程式設計。

jQuery 代替 JavaScript選擇HTML物件的方法：
* \$("#id") 這方式就可以用「#」抓到相應 ID 物件。
* \$(".className") 也可以用「.」抓到相應的class。
這跟CSS的寫法是一模一樣的，而且回傳值的一定是「Array」。
