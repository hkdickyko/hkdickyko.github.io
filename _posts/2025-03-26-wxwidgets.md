...
以下是标志的解釋：
 ...
 -x   分别选择语言 C 或 C++
 -E   使 gcc 仅运行预处理器，因此不进行编译
 -v   打印所有运行的命令，这是转储标准路径的关键
 \-   是要预处理的“输入文件”，按照惯例 - 代表 stdin（或 stdout，取决于上下文）；

echo \| 将一个空字符串提供给 gcc，这样就可以有效地预处理动态生成的空文件。

[解释 Linux 命令的互联网资源](https://explainshell.com/)


## 在 GCC C++ 中添加包含路径


gcc 是一个命令行程序，include 的标志是 -I

```sh
gcc -I需附加目录 <其他标志>
```

大型程序通常被分解成几个模块。除了使程序组织更加清晰之外，这种技术（称为 “模块化设计”）还允许将元素（模块）重用于不同的程序（例如，一个模块用于矩阵，另一个模块用于 “求一个数字” 等）。
