...
private:
  ...
  char *getPair(char *jsonStr, char *key) {
  ...
  void on_load(const std::string &msg) {
    ...
    sprintf(rValue, "readFile('%s')", content);
    sprintf(rValue, "readFile(\"%s\")", content);
    //sprintf(rValue, "document.getElementById('info').innerText = '%s';", content);
    // g_print(rValue);
    eval(rValue);
    free(rValue);
    free(content);
...
## Javascript 與 webview 之間的交流

程序跟網頁的交互，主要有兩個方面：
  - 程序需要調用網頁 Javascript 代碼。webkit2gtk 的內置 <font color="#FF1000">webkit_web_view_run_javascript</font> 函數即可解決
  - Javascript 需要調用程序的功能代碼。 webkit2gtk 的內置的 <font color="#FF1000">web extendsion</font> 擴展支持功能解決
