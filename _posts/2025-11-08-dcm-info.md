...
$$
\because q = \begin{bmatrix} q_w & q_x & q_y & q_z \end{bmatrix}
$$
...
$$
\therefore C_R^b = \begin{bmatrix}
 q_w^2 + q_x^2 - q_y^2 - q_z^2 & 2(q_xq_y \color{red}{+} q_wq_z) & 2(q_xq_z \color{green}{-} q_wq_y) \\
2(q_xq_y \color{green}{-} q_wq_z) &  q_w^2 - q_x^2 + q_y^2 - q_z^2 & 2(q_yq_z \color{red}{+} q_wq_x) \\
2(q_xq_z \color{red}{+} q_wq_y) & 2(q_yq_z \color{green}{-} q_wq_x) &  q_w^2 - q_x^2 - q_y^2 + q_z^2
\end{bmatrix}
$$
...
$$
\therefore C_b^R = \begin{bmatrix}
 q_w^2 + q_x^2 - q_y^2 - q_z^2 & 2(q_xq_y \color{green}{-} q_wq_z) & 2(q_xq_z \color{red}{+} q_wq_y) \\
2(q_xq_y \color{red}{+} q_wq_z) &  q_w^2 - q_x^2 + q_y^2 - q_z^2 & 2(q_yq_z \color{green}{-} q_wq_x) \\
2(q_xq_z \color{green}{-} q_wq_y) & 2(q_yq_z \color{red}{+} q_wq_x) &  q_w^2 - q_x^2 - q_y^2 + q_z^2
\end{bmatrix}
$$

注：$C_R^b、C_b^R$ 矩阵分别是沿对角轴翻转

### 如四元数顺序为 - xyzw

$$
\because q = \begin{bmatrix} q_x & q_y & q_z & q_w \end{bmatrix}
$$


$$
(C_R^b)_{wxyz} \Leftrightarrow (C_b^R)_{xyzw}，\\
(C_b^R)_{wxyz} \Leftrightarrow (C_R^b)_{xyzw}，
$$
...
从 **NED** 到 **body** 可以用转置 (Transpose) 计算。对于旋转矩阵，转置即逆矩阵。

$$
C_{Body}^{NED} = \left ( C_{NED}^{Body} \right )^T，C_{NED}^{Body} = \left ( C_{Body}^{NED} \right )^T
$$
