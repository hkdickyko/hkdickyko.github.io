...
Android 應用要記住應用主線程。主線程非常繁忙，因為它要處理繪製 UI，響應用戶的交互，默認情況下執行我們寫下的大部分代碼。當需要更新 UI 的時候需要 “返回” 到主線程，因為只有主線程才可以更新應用 *UI*。最常用的方式是調用 Activity 的 runOnUiThread() 方法。Runnable 對像就能在 UI 程序中被調用。如果當前線程是 UI 線程，那麼行動是立即執行。如果當前線程不是 UI 線程，操作是發佈到事件隊列的 UI 線程。注意 UI 線程即是主線程。
...
在Android平台中需要反复按周期执行方法可以使用Java上自带的TimerTask类，TimerTask相对于Thread来说对于资源 消耗的更低，除了使用Android自带的AlarmManager使用Timer定时器是一种更好的解决方法。 我们需要引入import java.util.Timer; 和 import java.util.TimerTask;
