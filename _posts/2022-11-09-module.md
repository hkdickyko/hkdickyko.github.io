...
require 方法用於調用模塊。它的參數與 *define* 方法類似。它是用 ”Asynchronous Module Definition” 即縮寫為 **AMD**，意思就是”異步模塊定義”。 它採用異步方式加載模塊，模塊的加載不影響它後面語句的運行。 所有依賴這個模塊的語句，都定義在一個回調函數中，等到加載完成之後，這個回調函數才會運行。
