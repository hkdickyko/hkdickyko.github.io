...
|截止頻率 ( $ F_{c} $ ) | 設計值 | $ F_{c} = F_{p} + 0.5 \Delta F $ |
...
|正規化截止頻率 ( $ \Delta f_{c} $ ) | $ \Delta f_{c} = \frac{F_{c}}{f_{s}} $ | $ \Delta f_{c} = \frac { {2\times{10^3}} + 0.5 \times{200}{10\times{10^3}} = {\color{Purple}{0.21}} $ |
...
從 **窗函數及其對應的屬性** 表中可以找到 **選擇過濾函數** 的正規化過渡帶寬度 $ \Delta f_{n} = \frac {3.3}{N} $ ，其中 N 是濾波器係數的數量。 經過重新排列這個方程使 N 成為主體並插入歸一化過渡寬度的值，然後可以計算所需數量的濾波器係數。 
...
$$ h_{D}[0] = \frac {\omega_{c}} {\pi} = \frac {\color{Purple}{0.21}}{pi} = 0.06685 $$  
