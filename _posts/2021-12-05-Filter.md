...
有限衝激響應（Finite impulse response，縮寫FIR）濾波器計算公式

$$ y(n) = \sum_{k=0}^{N} h(k) \times x(n-k) $$

無限衝擊響（Infinite Impulse Response，縮寫IIR）濾波器計算公式

$$ y(n) = \sum_{k=0}^{N} h(k) \times x(n-k) + \sum_{j=0}^{P} b(j) \times y(n-j) $$
...
使用窗口法設計 **FIR** 濾波器時，通常首先定義理想的頻率響應濾波器的 $ H_(\omega) $ ，當獲得了理想的頻率響應，就需要從頻率響應的逆傅立葉變換中獲得理想的脈衝響應 $ h_[\pm M] $。 設計過程的下一步是根據濾波器的通帶 $ \delta_{p} $ 和阻帶 $ \delta_{s} $ 容差的規格選擇合適的窗函數。 **FIR** 濾波器的容差最終取決於用於加窗的窗函數，但在設計過程中，我們總是從假設 $ \delta_{p} $ ≈ $ \delta_{s} $ 開始。在 **窗函數及其對應的屬性** 如下表中找到了合適的解決方案。當設計者根據濾波器規範確定了合適的窗函數，那麼然後可以計算所需數量的濾波器係數 **N**。 設計過程的最終程序是確定實際濾波器係數 $ h_[\pm M] $ 通過將 **窗口函數的係數** 如下表與相應的理想值相乘脈衝響應。 

- 注意：FIR濾波器的係數是對稱的。

$$ h(n) = h_[\pm M] \times w[n] $$
...
### 窗口函數的係數 $ w[n] $, $ \omega_{c} = 2 \pi F_{c} $

|濾波器類型|$ h_{D}[n] $ , n ≠ 0| $ h_{D}[0],F_{c} $ , n = 0|$ h_{D}[0], \omega_{c} $ , n = 0 |
|:---:|:---:|:---:|:---:|
|低通| $ 2 F_{c} \frac {\sin ({n \omega_{c}})} {n \omega_{c}} $ | $ 2 F_{c} $ | $ \frac {\omega_{c}} {\pi} $ |
|高通| $ 1 - 2 F_{c} \frac {\sin ({n \omega_{c}})} {n \omega_{c}} $ | $ 1- 2 F_{c} $ | $ 1 - \frac {\omega_{c}} {\pi} $ |
|帶通| $ 2 F_{2} \frac {\sin ({n \omega_{2}})} {n \omega_{2}} - 2 F_{1} \frac {\sin ({n \omega_{1}})} {n \omega_{1}} $ | $ 2 F_{2} - 2 F_{1} $ | $ \frac{\omega_{c2}-\omega_{c1}}{\pi} $ | 
|帶阻| $ 1-\left[2F_{2}\frac{\sin({n\omega_{2}})}{n\omega_{2}}-2F_{1}\frac{\sin({n\omega_{1}})}{n\omega_{1}}\right\] $ | $ 1-\left[{2F_{2}-2F_{1}}\right] $ | $ 1-\frac{\omega_{c2}-\omega_{c1}}{\pi} $ |
...
- 使用窗口，確定合適的窗口函數併計算所需的濾波器數量係數來設計這個濾波器。 此外，確定相應的濾波器係數值 $ h[\pm M] , -M \leqslant n \leqslant M $ 因 $ N = (2M +1) $
...
$$ N = \frac {3.3} { \Delta f_{n} } = \frac{3.3}{\color{DarkOrange}{0.02}} = 165, \pm M = \frac {165 - 1} {2} = \pm 82 $$ 
...
$$ \omega_{c} = 2 \pi (\Delta F_{n} + 0.5 \Delta f_{n}) = 2 \pi ({\color{Blue}{0.2}} + 0.5 \times {\color {DarkOrange}{0.02}}) = 0.42 \pi $$

因此，低通濾波器的理想脈衝響應可以改寫如下：

$$ h[\pm M] = \frac {\sin ({0.42 \pi n})} {\pi n} $$

低通濾波器的在截止頻率 $ \omega_{c} $ 的脈衝響應：

$$ h[0] = \frac {\omega_{c}} {\pi} = \frac {\color{Purple}{0.21}}{\pi} = 0.06685 $$  
...
當 M=10 如下：因係數是對稱的 $ h[M] = h[-M], h[10] = h[-10] $ 。 注意: 三角形函數使用弧度格式 ( *Radian* )

$$ h[10] = \frac {\sin ({0.42 \pi 10})} {\pi 10} \times \left [ 0.54 - 0.46 \cos \left ( \frac{2 \pi  10} {164} \right ) \right ]  = 0.0187  \times 0.1133 = 0.0021 $$

從上面的例子，你可以根據 <font color="#FF0010">n</font> 計算所有參數 $ h[\pm M] $ 。

## 總結設計過程如下

 - 根據阻帶幅度響應容差 ( $ \delta_{s} $ ) 選擇窗口濾波函數
 - 根據正規化過渡帶寬度 ( $ \Delta f_{n} $ ) 及窗口濾波函數,計算濾波器係數的總數量 ( $ N = 2M + 1 $ ), 如公式 11
 - 計算選擇的窗口濾波函數公式 $ w[n] $ ,如公式 12
 - 根據濾波器類型從上表 **窗口函數的係數**,計算 $ h[n] $ 及  $ h[0] $ ,如公式 15 及 16
 - 計算 $ w[n] \times h[n], 0 \leqslant n \leqslant N - 1 $ ,如公式 19
...
選擇最佳係數 a 值是總是在應用程序中進行權衡。某些應用程序在啟動階段需要兩者。 那就是濾波器必須具有快速的直流阻塞響應，同時具有最小的正常操作期間較低頻率的衰減。在這方案，可以通過選擇較小的直流阻塞來實現目標啟動時過濾係數並切換到較大的係數一旦濾波器阻止所有或大部分直流偏移。
...
![Alt int_e](../assets/img/filter/dcb_f.png)
