...
窗口法設計 **FIR** 濾波器時，通常首先定義理想的頻率響應濾波器，當獲得了理想的頻率響應，就需要從頻率響應的逆傅立葉變換中獲得理想的脈衝響應 $ h(i) $ 。 設計過程的下一步是根據濾波器的通帶 $ \delta_{p} $ 和阻帶 $ \delta_{s} $ 容差的規格選擇合適的窗函數。 **FIR** 濾波器的容差最終取決於選擇的窗函數，但在設計過程中，我們總是從開始假設 $ \delta_{p} $ ≈ $ \delta_{s} $ 。 在 **窗函數及其對應的屬性** 根據下表找到合適的濾波器窗函數，那麼然後可以計算所需數量的濾波器係數 $ \color {Red}{N} $ 。 設計過程的最終程序是確定實際濾波器係數的數量 。
