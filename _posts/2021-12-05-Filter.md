...
直流阻斷器是數字波導建模 [86] 和其他應用中不可或缺的工具。B.4 通常需要去除在延遲線環路中循環的信號的直流分量。它通常也是多軌錄音中的重要工具，其中各個軌道中的直流分量可以累加並溢出混音。

直流阻斷器是一個由差分方程指定的小型遞​​歸濾波器

$$ y(n) = x(n) - x(n-1) + R y(n-1) $$  

其中 R 是一個通常介 0.9 和 1 之間的參數（對於 44.1 kHz 採樣率，R=0.995 是好的）。 傳遞函數如下：

$$ h(z) = \frac{1 - z^{-1}} {1 - Rz^{-1}} $$ 

因此，在 DC (z=1) 處有一個零，在 z=R 處的 DC 附近有一個極點。遠離直流時，極點和零點大致相互抵消。