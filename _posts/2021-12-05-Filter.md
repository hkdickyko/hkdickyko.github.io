...
|濾波器類型|$ h[\pm M] $ , n ≠ 0| $ h[0],F_{c} $ , n = 0|$ h[0], \omega_{c} $ , n = 0 |
|:---:|:---:|:---:|:---:|
...
- 使用窗口，確定合適的窗口函數併計算所需的濾波器數量係數來設計這個濾波器。 此外，確定相應的濾波器係數值 $ h[\pm M] $, $ N = (2M +1) $
...
在時域中，截斷是通過將脈衝響應與窗函數 $ w[n] $ 相乘來實現的。 因此濾波器係數 $ h[\pm M] $ = -10, 10 可以通過以下方式獲得：
