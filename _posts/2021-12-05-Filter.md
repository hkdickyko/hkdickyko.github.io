...
$$ h_{n} = h(n-i) \times w(n-i),  i \in (-M, M) $$
...
- 確定合適的窗口函數併計算所需的濾波器係數數量來設計這個濾波器。 此外，確定相應的濾波器係數值 $ h(n-i) i \in (-M, M), N = (2M +1) $
...
$$ w(n) = 0.54 - 0.46 \cos \left ( \frac{\pi  n} {82} \right ) , M = 82 $$
