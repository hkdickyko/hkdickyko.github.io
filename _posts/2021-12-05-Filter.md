...
|濾波器類型|$ h[n] $ , n ≠ 0| $ h_[\pm M],F_{c} $ , n = 0|$ h[0], \omega_{c} $ , n = 0 |
|:---:|:---:|:---:|:---:|
...
$$ \pm M = \frac {165 - 1} {2} = \pm 82 $$ 

根據設計標準，**N** 參數數量為 <font color="#FF0010">165</font> 個，<font color="#FF1000">注意</font>：如果這是偶數，則應添加 1。

$$ W[n] = 0.54 - 0.46 \cos \left ( \frac{2 \pi  n} {N - 1} \right ) $$

$$ 0.54 - 0.46 \cos \left ( \frac{2 \pi  n} {164} \right ) , N = 165 $$

低通濾波器窗口函數的係數的簡化，因 $ \omega_{c} = 2 \pi F_{c} $ :

$$ h[\pm M] = 2 F_{c} \frac {\sin ({n \omega_{c}})} {n \omega_{c}} = 2 F_{c} \frac {\sin ({n \omega_{c}})} {n 2 \pi F_{c}} $$

$$ h[\pm M] = \frac {\sin ({n \omega_{c}})} {n \pi } $$ 

其中正規化角截止頻率，因 $ \omega_{c} $ 由以下等式定義:

$$ \omega_{c} = 2 \pi (\Delta F_{n} + 0.5 \Delta f_{n}) = 2 \pi ({\color{Blue}{0.2}} + 0.5 \times {\color {DarkOrange}{0.02}}) $$
...
低通濾波器的在截止頻率 $ \omega_{c} $ 的脈衝響應即 $ h[0] $ ：

$$ h[0] = \frac {\omega_{c}} {\pi} = \frac {\color{Purple}{0.21}}{\pi} $$
