...
首先我們需要從 **窗函數及其對應的屬性** 中選擇一個合適的窗函數，它具有最窄的 $ \Delta f_{n} $ 但滿足 $ \delta_{s} $ 規格為 <font color="#FF1000">0.00316</font>。 窗函數及其對應的屬性最接近的規格是具有 $ \delta_{s} $ 的漢明窗 = 0.0022。 從 **窗函數及其對應的屬性** 表中可以找到正規化過渡帶寬度 $ \Delta f_{n} = \frac {3.3}{N} $ ，其中 N 是濾波器係數的數量。 經過重新排列這個方程使 N 成為主體並插入歸一化過渡寬度的值，然後可以計算所需數量的濾波器係數。 

對於 **FIR** 濾波器，係數的數量必須是<font color="#FF1000">奇數</font>，所以不要忘記四捨五入到最接近的奇數。

$$ N =  \frac {3.3} { \Delta f_{n} } = \frac{3.3}{0.02} = 165 $$ 

根據設計標準，**N** 參數數量為 <font color="#FF0010">165</font> 個
