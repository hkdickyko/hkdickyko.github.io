...
$$ y(n) = \sum_{k=0}^{N} h_{k} \times x(n-k) $$

$$ y(0) = h_{0}\times x(0) +  h_{1}\times x(0-1) +  h_{2}\times x(0-2) + ... + h_{k}\times x(0-k) $$

無限衝擊響應（Infinite Impulse Response，縮寫IIR）濾波器計算公式

$$ y(n) = \sum_{k=0}^{N} h_{k} \times x(n-k) + \sum_{j=0}^{P} b_{j} \times y(n-j) $$

理想濾波器的脈衝響應 ℎ_{n} 是無限的，並且在 $ n = 0 $ 對稱。基於以上原因。所以將係數設為 $ N = 2M +1 $ 使其對稱。

$$ y(n) = \sum_{k=-M}^{M} h_{k} \times x(n-k) $$
