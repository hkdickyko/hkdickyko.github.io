...
class esp32IO:
  ...
  def aht10(self):
    ...
    humidty=(__humidty_raw/AHT_CONST)*100
    temperature=(__degree_raw/AHT_CONST)*200-50;
    factor = (math.log(humidty, 10) - 2) / 0.4343 + (17.62 * temperature) / (243.12 + temperature)
    dewpoint = 243.12 * factor / (17.62 - factor)
    return (temperature, humidty, dewpoint)
...
def updateStatus(jlist):
  ...
  if "btn3" in jlist.keys():
    info = devices.aht10()
    web.html_text("degree",str(info[0]))
    web.html_text("RH",str(info[1]))
    web.html_text("dp",str(info[2]))
    web.html_update()
