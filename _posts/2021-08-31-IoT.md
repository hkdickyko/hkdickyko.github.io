...
class webserver:
  ...
  def loop(self):
    response="Error!"
    while True:
      conn,addr=self.__webserver.accept()
      if not self.__html is None:
        response=self.__html
      request=conn.recv(1024)
      request=str(request)
      if self.__jSonStr is None:
        pt0=request.find("{")
        pt1=request.find("}")+1
        if pt0 < 10:
  ...
  <script>
    $(function() {
      ...
      function buildBtn(ID, bName, jsonObj){
        var jString="<a href='?"+JSON.stringify(jsonObj)+"'\"><button class=\"button\">" + bName + "</button></a>";
        $("#"+ID).append(jString);
      }

      var jsonObj={'led_1':'on','led_2':'off','led_3':'on'};
      buildBtn("b1", "Action 1", jsonObj);

      var jsonObj1={'led_1':'off','led_2':'on','led_3':'off'};
      buildBtn("b2", "Action 2", jsonObj1);
...
main.py - 處理最小網絡功能的示例 (*8080*是你在 *WiFi* 路由器地址中設置的轉發端口)
