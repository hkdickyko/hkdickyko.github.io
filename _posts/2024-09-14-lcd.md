...
void lcd_init (void)
{
    usleep(50000);  // at least 40ms after power rises
    lcd_send_cmd(C_BIT8);
    usleep(4500);   // at least 4.1ms
    lcd_send_cmd(C_BIT8);
    usleep(4500);   // at least 4.1ms
    lcd_send_cmd(C_BIT8);
    usleep(4500);   // at least 40us

    lcd_send_cmd(C_BIT4);
    usleep(200);
    lcd_send_cmd(C_4bitL2DOT7); //Function set
    usleep(1000);
    lcd_send_cmd(C_OFF); //Display off & Cursor off
    usleep(50);
    lcd_send_cmd(C_CLEAR); //Clear display at least 1.64ms
    usleep(2000);    
    lcd_send_cmd (C_CUR_R);
    usleep(50);
    lcd_send_cmd(C_ON); //Display on, Cursor off, Blink off
    usleep(50);
}
...
uint8_t flag= 1;
  while(flag)
  {
   ESP_ERROR_CHECK(i2c_master_receive(dev_handle, &flag, sizeof(flag), -1));
  }
}
