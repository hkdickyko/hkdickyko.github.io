...
B 为 1 时，光标所指字符闪烁。当 f<sub>cp</sub> 或 f<sub>osc</sub> 为 250 kH<sub>z</sub> 时，闪烁显示为以 409.6 ms 间隔的速度在所有空白点和显示字符之间切换。光标和闪烁可设置为同时显示。（闪烁频率根据 f<sub>cp</sub> 或 f<sub>OSC</sub> 的倒数而变化。例如，当 f<sub>cp</sub> 为 270 kH<sub>z</sub> 时。

$$
當 f_{osc} 不等於250kH_z, \\
新执行时间 = 旧执行时间 \times \frac {250kH_z}{f_{osc}}\\
新执行时间 = 409.6 ms × \frac {250}{270} = 379.2 ms
$$
