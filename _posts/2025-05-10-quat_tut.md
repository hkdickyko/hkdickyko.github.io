...
void EulerZYX(double roll, double pitch, double yaw, double RPY[3]);
...
void Qu_print(char *info, Quat_t * q)
{
	printf("%s:w:%5.5f, x:%5.5f, y:%5.5f, z:%5.5f\n", info, q->w, q->v[0], q->v[1], q->v[2]);
}

void Euler_print(char *info, Euler_t * e)
{
	printf("%s: Roll:%5.2f, Pitch:%5.2f, Yaw:%5.2f\n", info, e->Roll, e->Pitch, e->Yaw);
}
...
double Qu_AxisAngle(Quat_t * q, double output[3])
{
	...
	if (divider != 0.0)			// Calculate the axis
	{
...
void Qu_toEulerZYX(Quat_t * q, double output[3])
{
	...
	if (fabs(sinp) >= 1)
		output[1] = copysign(M_PI / 2, sinp);	// use 90 degrees if out of
												// range
	else
		output[1] = asin(sinp);
	// Yaw (z-axis rotation)
...
{
	double axis[3];
	EulerZYX(euler_angle->Roll, euler_angle->Pitch, euler_angle->Yaw, axis);
...
void Qu_to_Eular(Quat_t * q, Euler_t * euler_angle)
{
	...
	euler_angle->Roll = TO_DEG(output[0]);
	euler_angle->Pitch = TO_DEG(output[1]);
	euler_angle->Yaw = TO_DEG(output[2]);
}
