...
這個 *截止*, *拐角* 或 *斷點* 頻率被定義為容抗和電阻相等的頻率點，$ R = X_c $。當這種情況發生時，輸出信號衰減到輸入信號值的 70.7% 或輸入的 $ -3dB = 20 log \frac {Vout}{Vin} $。雖然 $ R = X_c $ ，但輸出不是輸入信號的一半。這是因為它等於兩者的向量和，因此是輸入的 0.707。由於濾波器包含一個電容器，因此輸出信號的相位角 $ \phi $ 滯後於輸入信號的相位角，並且在 $ -3dB $ 截止頻率 $ ƒ_c $ 處的相位角為 $ \pm45^{\circ} $ 取決於過濾器類型。
