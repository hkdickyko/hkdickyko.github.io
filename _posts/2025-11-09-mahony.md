...
yaw = - np.arctan2(my*np.cos(r) - mz*np.sin(r), 
...
p = np.arctan2(-ax, az)
yaw = np.arctan2(mx*np.cos(p) + mz*np.sin(p),
      mx*np.sin(p)*np.sin(r) + my*np.cos(r) - mz*np.cos(p)*np.sin(r))
```

$$
\mathbf{R} \approx \mathbf{R}_y = \left (\lambda_1|e_3-\mathbf{Rv}_a|^2 + \lambda_2|\mathbf{v}_m^*-\mathbf{Rv}_m|^2 \right )
$$
