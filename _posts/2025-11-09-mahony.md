...
$$
\begin{bmatrix} h_{rx} \\ h_{ry} \\ h_{rz}  \end{bmatrix} = \begin{bmatrix} h_{rx}cos(\phi) + h_{ry}sin(\phi)sin(\theta) h_{rz}sin(\phi)cos(\theta) \\ h_{rx} cos(\theta) - h_{rz} sin(\theta) \\ -h_{rx}sin(\phi) + h_{ry}cos(\phi)sin(\theta) + h_{rz}cos(\phi)cos(\theta)  \end{bmatrix}
$$

![Alt X](../assets/img/3d/magC.png)


$$
\psi = tan^{-1} \left ( \frac {h_{rx} cos(\theta) - h_{rz} sin(\theta)}{h_{rx}cos(\phi) + h_{ry}sin(\phi)sin(\theta) h_{rz}sin(\phi)cos(\theta)} \right )
$$
