...
如果知道 B 相对于 A 的姿态四元数，及 C 相对于 B 的姿态四元数，即 C 相对于 A 的姿态四元数为以下公式：

$$
q_{C->A} = q_{B->A} ⊗ q_{C->B}
$$

$$
q_c \cdot q_a = [q_{c0},q_{c1},q_{c2},q_{c3}] ⊗ [q_{a0},q_{a1},q_{a2},q_{a3}]
$$
...
$$
\begin{bmatrix} \theta \\ \phi \\ \psi \\ \end{bmatrix} = \begin{bmatrix} tan2^{-1} \left (\frac{2(yz+wx)}{1-2(x^2+y^2)} \right) \\ 
