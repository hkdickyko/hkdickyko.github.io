...
async def receiver():
    ...
    while True:
        ...
        regex = re.compile(",")
        head = regex.split(GGA_sentence)
        headstr = head[0]
        if re.match(r'\$GNGGA', headstr):
            for y in GGA_sentence:
                sentence = my_gps.update(y)
                if sentence:
                    fileStr = "{:10.8f}, {:10.8f} -- {} : {}".format(my_gps.latitude[0], my_gps.longitude[0], my_gps.altitude, my_gps.satellites_in_use)
                    print(fileStr)
