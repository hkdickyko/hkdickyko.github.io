...
# 要链结的 Lua 头文件目录。相等于 -I 目录
target_include_directories(${PROG_NAME} PUBLIC ${LUA_SRC_DIR}/lua-5.4.8/src)

# 要链结的 Lua 动态程式库目录。相等于 -L 目录
target_link_directories(${PROG_NAME} PUBLIC ${LUA_SRC_DIR}/lua-5.4.8/build)

# 如要引入的外部程式库，可用以下例子，m 为 math 数学程式库。相等于 -lm
# target_link_libraries(${PROG_NAME} m)

# 如要改变编绎后文件名称，可用以下例子，相等于 -o lib_Name
# set_target_properties(${PROG_NAME} PROPERTIES OUTPUT_NAME "lib_Name")
```
