...
根据以上列出函数即，在以现脚本下目录中有 socket 及 mime 兩個目录，socket 目录下有 <font color="#FF1000">3</font> 個文件，分别为 core.so， serial.so 及 unix.so。 而在mime 目录中只有 core.so 这 <font color="#FF1000">1</font> 個文件。
