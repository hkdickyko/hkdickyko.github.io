...
\begin{bmatrix} m \\ c \end{bmatrix} = \left( \begin{bmatrix} 1 & 1 & . & . & . & 1 & 1  \\ x_1 & x_2 & . & . & . & x_{n-1} & x_n  \end{bmatrix} \cdot \begin{bmatrix} x_1 & 1 \\ x_2 & 1 \\ . & . \\ . & . \\ x_{n-1} & 1 \\ x_n & 1 \end{bmatrix} \right)^{-1} \begin{bmatrix} 1 & 1 & . & . & . & 1 & 1  \\ x_1 & x_2 & . & . & . & x_{n-1} & x_n  \end{bmatrix} \begin{bmatrix} y_1\\ y_2\\ . \\ . \\ y_{n-1} \\ y_n \end{bmatrix}
$$

备注：$ X, Y$  是已知矩阵 $ b $ 是需要计算矩阵。
...
$$
A_n = \tilde{x} = (m^T m)^{-1}m^T y，\\
z_n = m^Ty = \sum _{i=1}^{n} m_i y_i 
$$
...
$$
P_n = P_{n-1}-P_{n-1} \cdot m^T\left (I+m \cdot P_{n-1} \cdot m^T\right)^{-1}m \cdot P_{n-1}
$$
...
$$
A_n = A_{n-1}+ K_n \left (y_n-m \cdot A_{n-1}\right)
$$
...
= \left(x_1+x_2+\cdots x_{n-1}\right)+x_{n}=(n-1)\times A_{n-1}+x_{n}
$$
...
= A_{n-1} + \frac{1}{n \cdot m} \left(y_n - m \cdot A_{n-1}\right)\\
