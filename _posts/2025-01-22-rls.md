...
上文可得，$\tilde{x} = (m_n^T m_n)^{-1}m_n^T y_n $  ，设 $ R_n = m^T_n m_n $

$$z_n = m^Ty = \sum _{i=1}^{n} x_i y_i ，\tilde{x} = (R_n)^{-1} z_n$$
...
在 $ \tilde{x_{n+1}} $ 的表达式中，比较麻烦的是求 $(R_{n+1})^{-1} $，如果得到它，那么就能成功求得 $ \tilde{x_{n+1}} $，得到线性模型。
