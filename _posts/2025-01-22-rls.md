...
$$
Y = X \cdot \begin{bmatrix} m \\ c \end{bmatrix}
$$
...
其中
 - $Y$ 为求解函数 $f(x) = \begin{bmatrix} y_1\\.\\.\\.\\y_n\end{bmatrix} $ 的测量值。
 - $X = \begin{bmatrix} x_1 & 1\\.&.\\.&.\\.&.\\x_n & 1\end{bmatrix}$ 为样本输入矩阵的测量值。
 - $b$ 为求向量函数 $\begin{bmatrix} m \\ c \end{bmatrix}$ 。
...
$$
P_n = P_{n-1}-P_{n-1} \cdot X^T\left (I+X \cdot P_{n-1} \cdot X^T\right)^{-1}X \cdot P_{n-1}
$$
