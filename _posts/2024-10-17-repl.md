...
要定义命令行选项，必须为 <font color="#FF0000">arg_</font>xxx 所需的每种类型的参数（文字 (arg_lit)、整数 (arg_int)、双精度 (arg_dbl) 、字符串 (arg_str)、文件名 (arg_file), 日期(arg_date)、正则表达式 (arg_rex) 等）。创建一个结构体，并将它整理到参数表（argument table）的数组中。
...
此命令是 <font color="#FF0000">arg_</font>xxx<font color="#FF0000">n</font> 是初始化结构的方法。