...
因在 IoT 中一般为小型设备，在设计 C 程序是要注意编译器优化及共享变数问题。

### volatile

volatile 关键字告知编译器，变量的值可能会受到程序无法显式控制的外部因素的影响而改变。这可以<font color="#FF1000">防止编译器执行某些优化</font>，因为这些优化原本可能假定变量的值在两次访问之间保持不变。

**应用场景**

 - 内存映射硬件寄存器：与硬件交互时，由于硬件操作，寄存器中的值可能会异步变化
 - 中断服务例程 (ISR) 修改的变量：ISR 可以修改全局变量，主程序需要了解这些潜在的变化
 - 多线程环境中线程间共享的变量

### extern

extern 声明某个变量或函数定义在另一个源文件或翻译单元中，并且当前文件打算使用它。它是一种声明，而非定义，也不会为该变量分配内存。

**使用场景**

 - 跨多个源文件共享全局变量：在一个 C 文件中定义的全局变量可以在其他 C 文件中声明为 extern，以便它们可以访问该变量
 - 声明在其他地方定义的函数：虽然函数原型通常可以实现这一点，因此使用 extern 并不常见，但也可以显式地声明在另一个文件中定义的函数

**主要差异总结**

|特性|volatile|extern|
|:---:|:---:|:---:|
|主要关注点|编译器优化和外部修改|跨文件链接和可见性|
|内存分配|不直接影响内存分配；而是影响编译器访问内存的方式|不分配内存；表示定义在其他地方|
|作用域|适用于变量在其作用域内如何处理其值|主要涉及全局作用域和文件间通信|
|交互|与编译器的优化过程交互|与链接器交互以解析符号|
