...
|代理模式（Proxy）| 提供对象的替代品，可以控制对原始对象的访问|
|外观模式（Facade）|提供一个简化的接口，用于访问一个更复杂的子系统|
...
![Alt X](../assets/img/pic/godot2d.png)


### 碰撞物件

**Godot** 2D 提供四种碰撞物件（**Area2D**、**StaticBody2D**、**RigidBody2D**、**CharacterBody2D**），皆继自 CollisionObject2D。 下列其中三种同时为物理主体，进一步自 PhysicsBody2D


|继承|类名|描述|用途|
|:---:|:---|:---:|:---:|
|CollisionObject2D|<font color="#FF1000">Area2D</font>|侦测功能。也能用来覆写特定区域内的物理属性，如重力或阻尼|侦测物件重叠，并在有物体进入或离开时发出讯号|
|CollisionObject2D|PhysicsBody2D|受物理影响的 2D 物件|基底类|
|PhysicsBody2D|<font color="#FF1000">RigidBody2D</font>|物理模拟进行移动的 2D 物理体|2D 物理体|
|PhysicsBody2D|<font color="#FF1000">CharacterBody2D</font>|主体提供碰撞侦测，但不会有物理模拟。所有移动及碰撞回应都须以程式码实作|移動的角色|
|PhysicsBody2D|<font color="#FF1000">StaticBody2D</font>|静态主体是不会被物理引擎移动的物件。它会参与碰撞侦测，但不会因碰撞而产生移动|场景环境、不动的物件等|
|StaticBody2D|AnimatableBody2D|无法被外力移动的 2D 物理物体。手动移动时，会对线速度和角速度进行估算，并用于影响其路径上的其他物理物体|移动平台、门等|
|Node2D|AnimatedSprite2D|包含多张纹理，可用作动画帧。可以导入图像文件|提供动画帧|
|Node2D|Camera2D|强制屏幕跟随该节点滚动|场景的相机|
|Node2D|RayCast2D|空间中的射线|用于查找第一个相交的碰撞|
|Node2D|CollisionShape2D|提供一个允许的编辑对象，并为对象赋予检测形状|转换对象为实体|
|Node2D|Line2D|可以选择设置纹理的 2D 折线|绘制一条 2D 折线|
|Node2D|Path2D|创建路径，遵循节点|绘制一条 2D路径|
|Node2D|ShapeCast2D|扫描空间中某个区域|用于检测碰撞|

## Godot 3D 实践

![Alt X](../assets/img/pic/godot3d.png)


### 碰撞物件

**Godot** 3D 提供四种碰撞物件（**Area3D**、**StaticBody3D**、**RigidBody3D**、**CharacterBody3D**），皆继自 CollisionObject3D。 下列其中三种同时为物理主体，进一步自 PhysicsBody3D

|继承|类名|描述|用途|
|:---:|:---|:---:|:---:|
|CollisionObject3D|<font color="#FF1000">Area3D</font>|侦测功能。也能用来覆写特定区域内的物理属性，如重力或阻尼|侦测物件重叠，并在有物体进入或离开时发出讯号|
|CollisionObject3D|PhysicsBody3D|受物理影响的 3D 物件|基底类|
|PhysicsBody3D|<font color="#FF1000">RigidBody3D</font>|物理模拟进行移动的 3D 物理体|3D 物理体|
|PhysicsBody3D|<font color="#FF1000">CharacterBody3D</font>|主体提供碰撞侦测，但不会有物理模拟。所有移动及碰撞回应都须以程式码实作|移动的角色|
|PhysicsBody3D|<font color="#FF1000">StaticBody3D</font>|静态主体是不会被物理引擎移动的物件。它会参与碰撞侦测，但不会因碰撞而产生移动|场景环境、不动的物件等|
|StaticBody3D|AnimatableBody3D|无法被外力移动的 3D 物理物体。手动移动时，会对线速度和角速度进行估算，并用于影响其路径上的其他物理物体|移动平台、门等|
|Node3D|AnimatedSprite3D|包含多张纹理，可用作 3D 动画帧。可以导入 3D 文件|提供动画帧|
|Node3D|Camera3D|强制屏幕跟随该节点滚动|场景的相机|
|Node3D|RayCast3D|3D 空间中的射线|用于查找第一个相交的碰撞|
|Node3D|CollisionShape3D|提供一个允许的编辑对象，并为对象赋予检测形状|转换对象为实体|
|Node3D|Skeleton3D|管理骨骼层级结构，包括姿势和动画|布娃娃物理|
|Joint3D|HingeJoint3D|种物理关节，用于限制 3D 物理体相对于另一个物理体绕轴旋转|旋转关节|
|GeometryInstance3D|MeshInstance3D|一个网块资源，并通过创建其实例将其添加到当前场景中|不规则形状7|
|Light3D|DirectionalLight3D|模拟覆盖整个场景的无限多平行光线|阳光或月光|