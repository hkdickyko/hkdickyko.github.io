...
四元數可以藉由相乘來完成連續旋轉，所以我們將把 Roll（$\theta$）, Pitch（$\phi$ ）, Yaw（$\psi$）分為三個四元數進行旋轉，最後再將它們串成同一個四元數。
...
\end{bmatrix} =
\begin{bmatrix}
 atan2(2(sz+xy),1-2(z^2+x^2)) \\
 asin(2(sz-yz)) \\
 atan2(2(sy+zx),1-2(x^2+y^2)) \\
\end{bmatrix}
$$
...
只是在做旋轉時用的四元數被稱為旋轉四元數。 旋轉四元數的構造需要2個東西：笛卡爾坐標系下的旋轉軸 $\hat{v}$ 的 $3D$ 向量及繞著這個旋轉軸的旋轉角度 $ \theta $ 的弧度。
