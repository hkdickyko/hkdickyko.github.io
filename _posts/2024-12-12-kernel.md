...
![Alt X](../assets/img/linux/proc.png)


|档名|文件内容|
|:---:|:---:|
|/proc/cmdline|加载kernel时所下达的相关参数！查阅此文件，可了解系统是如何启动|
|/proc/cpuinfo|本机的CPU的相关资讯，包含时脉、类型与运算功能等|
...
|/proc/meminfo|使用free列出的内存资讯，在这里也能够查阅到|
/proc/modules|目前我们的Linux已经加载的模块列表，也可以想成是驱动程序|
/proc/mounts|系统已经挂载的数据，就是用mount这个命令呼叫出来的|
/proc/swaps|到底系统挂加载的内存在哪里？使用掉的partition 就记录在此|
/proc/partitions|使用fdisk-1会出现目前所有的partition.在这个文件当中也有纪录|
/proc/pci|在PCI汇流排上面，每个装置的详细情况！可用lspci来查阅|
/proc/uptime|就是用uptime的时候，会出现的资讯|
