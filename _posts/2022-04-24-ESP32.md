...
// 通用输入输出端口
void initGPIO();
void setGPIO(gpio_num_t GPIO_PIN, bool onFalg);
...
void list(const char *path, const char *match, const char *partition_Name)
{
  ...
  while ((ent = readdir(dir)) != NULL)
  {
    ...
    if ((match == NULL) || (fnmatch(match, tpath, (FNM_PERIOD)) == 0))
    {
      ...
      if (statok == 0)
      {
        ...
        //        strftime(tbuffer, 80, "%d/%m/%Y", tm_info);
      }
      else
        sprintf(tbuffer, "-");
      ...
      sprintf(strValue + n, "{'檔案類型':'%c','檔案大小':'%s','日期/時間':'%s','檔案名稱':'%s'},", type, size, tbuffer, ent->d_name);
    }
    n = strlen(strValue);
  }
  sprintf(strValue + n, "]");
  ...
  if (total)
  {
    ...
    if (total < (1024 * 1024))
      ...
      printf("   %6d KB", (int)(total / 1024));
    else
      printf("   %6d MB", (int)(total / (1024 * 1024)));
    printf(" in %d file(s)\n", nfiles);
  }
  printf("---------------------------------------------------\n");
  closedir(dir);
...
char *fileRead(const char *fileName_Path)
{
  ...
  if (f != NULL)
  {
    ...
    content[i] = '\0';
    fclose(f);
    printf("File %s was read, bytes:%d.\n", fileName_Path, i);
    return content;
  }
...
char *readSpiffs(char *fileName)
{
    ...
    // list("/spiffs", "*.*", conf.partition_label);
    spiffsClose(conf);
    return content;
}

void gpioControl(char *resp_str)
{
    esp_vfs_spiffs_conf_t conf;
    initGPIO();
    if (strcmp(resp_str, "LED_1O") == 0)
    {
        printf("\nOn1");
        setGPIO(12, 1);
    }
    if (strcmp(resp_str, "LED_1F") == 0)
    {
        printf("\nOff1");
        setGPIO(12, 0);
    }
    if (strcmp(resp_str, "LED_2O") == 0)
    {
        printf("\nOn2");
        setGPIO(13, 1);
    }
    if (strcmp(resp_str, "LED_2F") == 0)
    {
        printf("\nOff2");
        setGPIO(13, 0);
    }
    if (strcmp(resp_str, "List") == 0)
    {
        conf = spiffsInit("/spiffs", "storage");
        list("/spiffs", "*.*", conf.partition_label);
        spiffsClose(conf);
...
{
    char *content = readSpiffs("/spiffs/sample.html");
    char *Actual_IP = getIP();
    char *outContent = replaceStr(content, "IP_ADDRESS", Actual_IP);
    printf("%s",outContent);
    httpd_resp_send(req, outContent, HTTPD_RESP_USE_STRLEN);
    free(content);
    free(Actual_IP);
    free(outContent);
    char *resp_str = getUrlStr(req);
...
esp_err_t misc_get_handler(httpd_req_t *req)
{
    esp_vfs_spiffs_conf_t conf;
    conf = spiffsInit("/spiffs", "storage");
    char fileName[7 + strlen(req->uri)];
