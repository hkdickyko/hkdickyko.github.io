...
实际上，四元数的 $x、y$ 和 $z$ 分量定义了旋转轴方向的向量。而 $w$ 分量定义旋转角度。當 $w=0$ 称为恒等四元数，不会产生旋转。在这种情况下<font color="#FF1000">旋转角为零</font>。

当另一四元数出现，因共同的参考点是 (0,0,0)。三点形成一个平面。而这平面的直角则形成一個主轴。则由两四元数之间会形成一个最短的弧形距离及一个角度。这即是四元数之间的旋转角。用球面线性插值。即可找到這路径中的随意 **t** 值的过度四元数分量。

简单理解相等于可以随意改变主轴。再去计算出其它四元数与主轴之间的角度关係。 