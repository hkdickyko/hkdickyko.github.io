...
实际上，四元数的 $x、y$ 和 $z$ 分量定义了垂直旋转轴方向的向量。而 $w$ 分量定义旋转角度。當 $w=0$ 称为恒等四元数，不会产生旋转。在这种情况下<font color="#FF1000">旋转角为零</font>。即下图的绿色向量。

当另一四元数出现。即下图的红色向量，因共同的参考点是 (0,0,0)。三点形成一个平面。即下图的灰色平面。而这平面的直角则形成一個主轴。即下图的蓝色轴。则由两四元数之间会形成一个最短的弧形距离及一个角度，即下同啲橙色弧形缐段。这即是四元数之间的旋转角。用球面线性插值。即可找到這路径中的随意 **t** 值的过度四元数分量。

![Alt x](../assets/img/3d/slerp-1.png)

简单理解相等于可以随意改变主轴向量方位。再去计算出其它四元数与主轴之间的角度关係。

请<font color="#FF1000">注意</font>:虽然是以圆形角度方法计算，但计算后的一系列四元数。会通过第一点及最终一点。中间每个四元数的長度可以不同。而角度就相对拉伸变改变。这也是使用四元数，在 3 维空间中的应用。其计算的方法较其它方法简单。