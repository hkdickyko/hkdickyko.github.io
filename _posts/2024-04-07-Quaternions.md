...
实际上，四元数的 $x、y$ 和 $z$ 分量定义了指向旋转轴方向的向量。 $w$ 分量定义沿该轴的旋转量。當 $w=0$ 称为恒等四元数，不会产生旋转。在这种情况下<font color="#FF1000">旋转角为零</font>。当另一四元数出现，因大家共同的参考点是 (0,0,0)。则由两四元数之间会形成一个最短的弧形距离及一个角度。这即是四元数之间的旋转角。用球面线性插值。即可找到這路径中的过四元数分量。