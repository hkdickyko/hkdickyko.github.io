...
LightUserData: 轻量级用户数据，相比于上面的FullUserData，它是轻量的，它只是一个指针，在 C 语言中对应的类型为 void*，占 4 或 8 个字节，Lua 不会管理这个指针的内存，全由使用者自己管理。
...
**总结如下**：
 ...
 - LightUserData 在 C 语言中进行内存分配与管理，Lua 只存储一个指针值。怎么回收需要 C 管理。
 - UserData 相比 light userdata 只是多了内存分配和 GC 时的内存回收。因为自动回收的特性，特别适合数据生命周期和 userdata 对象在 Lua 中存活周期一致的场景。
...
Light userdata 主要是 **lua_pushlightuserdata** 和 **lua_touserdata**。注意获取栈上的 userdata 或者 light userdata 对应的指针式相同的 API。
...
**C** 函数可以通过堆栈来和 Lua 交换数据，但有时候 C 函数需要在函数体的作用域之外保存某些 Lua 数据。可用 **全局变量** 或 **static** 变量，但这样做有缺点如下：

 - 为 Lua 设计 C 函数库时，导致不可重入
 - 不是所有的 Lua 值都能很好的保存到 C 变量中

因此 Lua 提供了一个独立的被称为 registry 的表，但是 Lua 代码本身不能访问它。

### registry 全局注册表

一个普通的 Lua 表，使用假索引 (pseudo-index) **LUA_REGISTRYINDEX** 访问。C 代码可以访问，Lua 代码不能访问。

 用途：解决 C 函数保留全局 Lua 值的问题。
 
 注意：所有的 C 库共享相同的 registry，所以对于 key 的命名需要具有全局唯一性。

```lua
-- 获取 registry 表键值 "KEY" 对应值的方法
lua_pushstring(L, "KEY");
lua_gettable(L, LUA_REGISTRYINDEX);

-- 创建及读取 registry 表键值 "3D" 对应值的方法
const char *key = "3D";
...
通过一个整数来唯一标识一个 Lua 数据对象，由两个函数 **luaL_ref** 和 **luaL_unref** 组成，避免函数冲突並将值保存到 registry 中。
...
注意：永远不要使用**数字**作为 registry 的 key，因为这种类型的 key 是保留给 reference 系统使用。
