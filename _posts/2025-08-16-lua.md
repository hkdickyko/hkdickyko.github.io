...
// 相等于 lua_createtable(0,0) 
void lua_newtable();
// 获取表中索引 k 的值，并压入栈。调用会触发 __index 方法， index 为表
void lua_getfield(int index, const char *k);
// 设置索引 k 的值，并弹出栈顶元素。调用会触发 __newindex 方法，index 为表
void lua_setfield(int index, const char *k);
// 获取全局变量 name，并压入栈顶
void lua_getglobal(const char *name);
// 弹出栈顶，并将其设置为全局变量 name 的值
void lua_setglobal(const char *name); 

// 弹出栈顶，并压入 t[k] 值，t 为索引 index 的表，k 为栈顶值
// index 为表。调用会触发 __index 方法 
void lua_gettable(int index);
// 设置表 t[k]=v，t 为索引 index 的表，v 是栈顶元素，k 为栈顶下面一个元素。例如 lua_settable(-3) —- table , “key” , “value” () ，表示栈顶。并弹出v(栈顶)和k(栈顶下面)元素，调用会触发 __newindex 方法
void lua_settable(int index) : 
// 与 gettable 一样，但调用的是原始索引方法
void lua_rawget(int index);
// 与 settable 一样，但调用的是原始索引方法
...
int lua_getmetatable(int index);
// 弹出栈顶，并将其设置为索引 index 值的 metatable
int lua_setmetatable(int index);
// 向 lua 注册名字为 name 的 f 方法。相当于
//#define lua_register(L,n,f) (lua_pushcfunction(L, f), lua_setglobal(L, n))
void lua_register(const char *name, lua_CFunction f);
```
### Lua和C简单交互代码
...
![Alt X](../assets/img/linux/stackindex.png)

 - 绝对索引是从 **1** 开始由栈底到栈顶依次增长的
 - 相对索引是从 **-1** 开始由栈顶到栈底依次递减的（在lua API函数内部会将相对索引转换为绝对索引）
 - 上图栈的容量为 7，栈顶绝对索引为 5
    - 有效索引范围为：[1, 5]
    - 可接受索引范围为：[1, 7]
 - Lua 虚拟机指令里寄存器索引是从 0 开始的，而Lua API里的栈索引是从 1 开始的，因此当需要把寄存器索引当成栈索引使用时，要进行 +1
