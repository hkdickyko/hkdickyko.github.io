...
// 弹出栈顶，并压入 index 指向的表，调用会触发 __index 方法 
void lua_gettable(lua_State *L, int index);
// 设置表 t[k]=v，t 为索引 index 指向的的表，v 是栈顶元素，k 为栈顶下面一个元素。例如 lua_settable(-3) —- table , “key” , “value” () ，表示栈顶。并弹出v(栈顶)和k(栈顶下面)元素，
// 调用会触发 __newindex 方法
...
**设置lua表值**
 ...
 - lua_gettable 压入表的键值，index 指向的表，弹出栈顶键值，并压入表的值

**读出 Lua 表的值到栈顶** lua_gettable(L, -2)

![Alt X](../assets/img/lua/lua_gettable.png)


**从栈顶值设置 Lua 表的值**  lua_settable(L, -3)

![Alt X](../assets/img/lua/lua_settable.png)
