...
常用的指令：
 ...
 - run (r) : 運行程序，當遇到斷點後，程序會在斷點處停止運行，等待用戶輸入下一步命令
 - continue (c) : 繼續執行，到下一個斷點停止（或運行結束）
 - next (n) : 單步跟踪程序，當遇到函數調用時，也不進入此函數體；此命令同 step 的主要區別是，step 遇到用戶自定義的函數，將步進到函數中去運行，而 next 則直接調用函數，不會進入到函數體內。
 - step (s) : 單步調試如果有函數調用，則進入函數；與命令n不同，n是不進入調用的函數的
 - until (u) : 當你厭倦了在一個循環體內單步跟踪時，這個命令可以運行程序直到退出循環體。
...
我們需要關注的選項有：
 ...
 - program：可執行文件的的路徑，相對路徑或絕對路徑都可以
 - args：運行程序時傳遞的參數
