...
|DIR-|步进方向输入负|
|A+|步进电机 A+ 连接|
|A-|步进电机 A- 连接|17HS4401S28BYJ48步进马达
|B+|步进电机 B+ 连接|
|B-|步进电机 B- 连接|
|DC+|电源正|
|DC-|电源负|
...
ENA 引脚的控制可以用来在不需要时关闭电机，以节省电力或防止电机过热。如果不使用 ENA 引脚，可以将 ENA+ 和 ENA- 直接连接到 GND，始终使能驱动器。
...
将驱动器设置为32细分，看驱动上的注释可知道转一圈需要6400次，计算分频系数，系统时钟为72M，那么可以算出72000000/500/100=1440，即1440次/秒；所以在这就可以得到电机转动一圈需要的时间为6400/1440大约就是4.4秒。这样就能实现不同电机的转速转动。现在的程序只是通过在程序中修改电机旋转方向以及分频系数来改变转速。
...
即步进电机在空载情况下能够正常启动的脉冲频率，如果脉冲频率高于该值，电机不能正常启动，可能发生丢步或堵转。在有负载的情况下，启动频率应更低。如果要使电机达到高速转动，脉冲频率应该有加速过程，即启动频率较低，然后按一定加速度升到所希望的高频（电机转速从低速升到高速）。

    - 启动频率 = 启动转速 × 每转多少步。

空载启动转速就是步进电机不通过加减速不负载直接转动起来。当步进电机转动时，电机各相绕组的电感将形成一个反向电动势;频率越高，反向电动势越大。在它的作用下，电机随频率(或速度)的增大而相电流减小，从而导致力矩下降。

　　假设：需要减速器的总的输出转矩是 **T1**，输出的转速是 **N1**，减速比是 **5：1**，步进电机的步进角度是 **A**，那么电机的转速是：**5xN1**，那么电机的输出转矩应该是 **(T1)/5**，电机的工作频率应该是

$$
　　工作频率 = 减速比 \times 转速 \times \frac {360}{步进角度} = 5 \times N1 \times \frac {360}{A}
$$

所以应该看矩频特性曲线：坐标点[(T1)/5，5*(N1)*360/A]是不是在频特性曲线(启动矩频曲线)的下面。如果在矩频曲线的下面，可以选择这个电机。如果是在矩频曲线上面，则不能选择这个电机，因为会失步，或者根本就不能转动。

![Alt 矩频特性曲线](../assets/img/esp/MT1-29_f01.gif)

补充：是否确定了工作状态，需要的最大转速确定了吗，如果确定了，那就可以根据上面提供的公式进行计算，(根据转动的最大速度，和负载的大小，你就可以确定你现在选用的步进电机是否适合，如果不适合你也应该知道要选用什么样的步进电机了)。
...
关于步距角，比如说你是 A-B-C-D-A 单四拍控制，那么步距角就是一个 *A* 走过的角度，关于最大启动频率，其指的是 **A-B** 之间的间隔频率，手册里给的都是 > 于某个值，但是在实际应用时感觉应该给的值就是最大值，例如 **>250PPS**，那么 **A** 之后的 **延迟间隔** 就满足 
$$
 间隔频率 = \frac {1}{延迟间隔} <= 250, 延迟间隔 >=4ms
$$
...
由于 **ESP32** 预设的 **portTICK_PERIOD_MS** 为 **100Hz**，如果要比预设的运行速度更快，则需要更改 **CONFIG_FREERTOS_HZ** 参数，但该值最大只能改变为 **1kHz**。
