...
|quit|q|结束 GDB 侦错会话|
|clear \<n\>|clear \<n\>|指定位置 n（行号或函数名）清除中断点|
|delete \<n\>|delete \<n\>|指定 n（中段点编号）清除中断点，也可擦除以下 display 及 watch|
|display \<name\>|display \<name\>|指定 name（变数）每步自动列出|
|watch \<name\>|watch \<name\>|指定 name（变数）每次更新列出|
...
注意：可用 <font color="#FF1000">Ctrl+c</font> 终止程序。

详细可参考以下连结：[C 工具 (Linux)](https://hkdickyko.github.io/%E8%BD%AF%E4%BB%B6/ctools)