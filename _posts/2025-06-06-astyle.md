...
|--style=java|-A|java Java 样式格式/缩排。附加的括号|
|--style=k&r/--style=k/ |-A3|k&r Kernighan & Ritchie 样式格式/缩排。Linux 括号|
|--style=stroustru |-A4 |stroustrup Stroustrup 样式格式/缩排|
|--style=whitesmit |-A5| whitesmith Whitesmith 样式格式/缩排 断开的缩排括号|
|--style=banner |-A6 |Banner 样式格式/缩排|
|--style=gnu |-A7 |gnu GNU 样式格式/缩排。缩排 2 个空格|
|--style=linux |-A8| linux风格 GNU 风格的格式/缩排。缩排 8 个空|
...
- \`command\` (旧式反引号形式的替换) 等于 \$(command) 。在 A=\`B\` : \` 中输入的任何 shell 命令的将输出到 A。\$() 语法（通常称为命令替换）会在子 shell 中执行命令，并用该命令的输出替换 \$() 表达式。这允许将一个命令的输出用作另一个命令的参数(即赋值给变量)。