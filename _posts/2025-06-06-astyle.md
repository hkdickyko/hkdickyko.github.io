...
最后四个指令用作增加额外标签
 ...
 - new-inline-tags
    - 不以新行开头的元素称为内联元素 (new-inline-tags)。它不以新行开头，并且仅占用必要的宽度（即定义 HTML 元素的标签所定义的空间），而不会中断内容流。\<a\>,\<abbr\>,\<acronym\>,\<b\>,\<bdo\>,\<big\>,\<br\> 是一些内联元素的示例。
 - new-blocklevel-tags
 ...
 - new-pre-tags
   - new-pre-tags 标签代表预格式化文本块。标签将保持输出格式与输入格式相同。当要显示文本时，如果排版格式会影响内容的含义（例如代码片段和诗歌），它会非常方便。
