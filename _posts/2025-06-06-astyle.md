...
|长指令名|短指令名|说明|
|:---:|:---:|:---|
|--style=allman/<br>--style=bsd|-A1| allman Allman 样式格式/缩排。 断开的括号|
|--style=java|-A2 |java Java 样式格式/缩排。 附加的括号| |--style=k&r/--style=k/r |-A3| k&r Kernighan & Ritchie 样式格式/缩排。  Linux 括号|
|--style=stroustrup |-A4 |stroustrup Stroustrup 样式格式/缩排。  Stroustrup 括号|
|--style=whitesmith |-A5| whitesmith Whitesmith 样式格式/缩排。 断开的缩排括号。 缩排的类块和开关块|
|--style=banner |-A6 |Banner 样式格式/缩排。 附加的缩排括号。 缩排的类块和开关块|
|--style=gnu |-A7 |gnu GNU 样式格式/缩排。 括号断开，程式码区块缩进，缩排 2 个空格|
|--style=linux |-A8| linux风格 GNU 风格的格式/缩排。  Linux 括号，缩排 8 个空|
...
|长指令名|短指令名|说明|
|:---:|:---:|:---|
|--indent-classes |-C|缩进类定义中的 public private protected 标签缩排 'class' 区块，以便内部的 'public:'、 'protected:' 和 'private:' 标头相对于 class 区块缩排|
|--indent-switches |-S|缩进 switch 中的 case 块, case 和  switch 不在同一列|
|--indent-cases|-K|缩进 case 下面的语句。 从「case XXX:」标题开始缩排 case 区块。 未包含在区块中的 case 语句不缩排|
|--indent-blocks|-G|给块添加缩进，包括大括号为整个区块（包括括号）添加额外的缩排|
|--indent-brackets|-B|给大括号添加缩进，为 “{” 和 “}” 区块括号添加额外的缩排。  |
|--indent-namespaces|-N|缩进命名空间定义行，缩排命名空间区块的内容|
|--indent-labels|-L|标签缩进，使其比目前缩排等级少一个缩进，而不是完全向左对齐（这是预设值）|
|--indent-preprocessor|-w|多行宏定义缩进，缩排多行#define 语句|
|--max-instatement-indent=#|-M#|跨行缩进,比如函数定义中的参数处在多行,那么参数缩进到同一列，连续语句中最多缩排 # 个空格，相对于上一行|
...
|长指令名|短指令名|说明|
|:---:|:---:|:---|
|--break-blocks|-f|没有关系的块插入空白行，类，标签(不包括函数块)|
...
|--keep-one-line-blocks|-O|大括号中的单行语句不被拆分|
...
|--suffix=none|-n|不备份文件，默认生成 .orig 文件|
...
```
 ...
 - 若是值內有空白則需要使用 '' 或 "" 包裹起來
 - 比較條件需要放在 [] 中，前後要留空白。例子 if [ $x -gt $y ]; then
