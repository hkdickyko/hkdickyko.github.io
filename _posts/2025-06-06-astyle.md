...
${filepath##*/} 从开头删除最长的 */ 匹配项，只保留带扩展名的文件名。

${filename_with_ext%.*} 从文件名末尾删除最短的 .* 匹配项，从而有效地删除扩展名。

${filename##*.} 从文件名开头删除最长的 *. 匹配项，从而有效地提取扩展名。


%：字符串操作
 - 用于参数扩展，删除字符串中的前缀或后缀。例如，${var%suffix} 会从 var 中删除最短匹配的后缀。
...
字符串操作：
 - ${variable#pattern}：从变量开头删除模式的最短匹配。
 - ${variable##pattern}：从变量开头删除模式的最长匹配。
 - ${variable%pattern}：从变量末尾删除模式的最短匹配。
 - ${variable%%pattern}：从变量末尾删除模式的最长匹配。
 - ${variable/pattern/replacement}：用替换项替换模式的第一个匹配。
 - ${variable//pattern/replacement}：用替换项替换模式的所有匹配。
