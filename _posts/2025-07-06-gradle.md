...
## build.gradle 和 settings.gradle 的关系

 - build.gradle：定义每个模块的构建逻辑和依赖。
 - settings.gradle：定义项目的结构和模块。
    - 每次只会执行一个 settings.gradle 脚本。它会根据一个 Settings 物件进行评估。透过这个 Settings 对象，可以为建置中新增子项目、从命令列修改参数，以及存取 Gradle 物件来注册生命周期处理程序。 因此，如果设定与建置相关，而不一定与专案相关，或者需要在包含可能的子专案之前执行逻辑，则可以使用 settings.gradle。
 - gradle.properties 文件是一个简单的 Java 属性文件，它的特殊之处在于它会自动包含在 Project 对象的作用域中。它是一个简单的键值存储，仅允许字符串值（因需要自行拆分列表或数组）。可以将 gradle.properties 文件放在以下位置：
    - 直接放在项目目录中（用于项目相关的值）
    - 放在用户主目录下的 .gradle 目录中（用于用户或环境相关的值）
