...
Gradle 需要特定的 Java 开发工具包 (JDK) 版本来运行 Gradle 守护程序本身，但它可以使用工具链为较旧或较新的 Java 版本编译代码。可参考一下 Gradle 兼容性矩阵。Gradle 经过 Android Gradle Plugin 8.4 到 8.11 的测试。


[Gradle 兼容性矩阵](https://docs.gradle.org.cn/current/userguide/compatibility.html)


## 关键文件和目录

一个典型的 Gradle 项目包含以下基本文件和文件夹：

|文件/目录|描述|
|:---:|:---|
|build.gradle(.kts) |项目或子项目的主构建脚本。它使用 Groovy (.gradle) 或 Kotlin (.gradle.kts) DSL 配置插件、定义任务并声明依赖项。|
|settings.gradle(.kts) |设置文件定义构建名称并确定项目层次结构，声明构成构建的任何子项目。它通常位于根目录中。|
|gradle/wrapper|包含 Gradle Wrapper JAR 文件和属性文件 (gradle-wrapper.properties)。这确保每个人都使用相同的 Gradle 版本，从而使构建在不同机器上保持一致。|
|gradlew / gradlew.bat|分别是用于 Unix/macOS 和 Windows 的 Gradle Wrapper 脚本。 这些是用于运行 Gradle 任务的命令行工具（例如，./gradlew build）。|
|src/main/java|生产环境 Java 代码的默认源目录（或 src/main/kotlin 等，具体取决于所使用的语言插件）。|
|src/test/java|测试代码的默认源目录。|
|build|Gradle 生成所有文件（例如编译后的类、JAR 文件和测试报告）的默认输出目录。版本控制系统通常会忽略此目录。|
|.gradle/|Gradle 用于缓存和其他临时文件的隐藏目录，以支持增量构建等功能。版本控制系统通常也会忽略此目录。|
|buildSrc|一个可选的专用目录，用于组织和重用项目子项目中的自定义构建逻辑和约定插件。|
...
每个 build.gradle(.kts) 文件通常都遵循特定的块式结构：

 - plugins { ... }
    - 声明添加特定任务和功能所需的插件（例如，java、application、com.android.application）。
 - repositories { ... }
    - 指定外部库的存放位置，例如 mavenCentral() 或 google()。
 - dependencies { ... }
    - 列出不同配置所需的库，例如生产环境的 implementation 或测试环境的 testImplementation。
 - tasks
    - 自定义或内置的工作单元，例如 compileJava 或 test。


## Gradle 插件声明基础

Gradle 插件通过 plugins 块进行声明，每个插件都有一个唯一的 ID 和可选的版本号。在 Android 开发中， com.android.application 插件是构建 Android 应用程序的核心插件。

### 插件声明语法

```
plugins {
    id '插件 ID' version '版本号' apply false
}
```

 - apply false 的用法
     - apply false 是 Gradle 插件声明中的一个特殊选项，它告诉 Gradle 仅声明插件但不立即应用。这在多模块项目中特别有用，因为它允许在顶层 build.gradle 文件中统一管理插件版本，而在需要插件的模块中单独应用。
 - apply false 的优势
     - 统一管理：在顶层build.gradle中声明插件版本，便于统一管理和更新。
     - 灵活应用：在需要插件的模块中单独应用，避免不必要的插件加载。
     - 减少重复：避免在每个模块中重复声明插件版本，减少代码冗余。
...
一般来说，项目的第三方依赖都是通过 dependencies 节点来配置我们第三方的下载路径的，常见的有 implementation，api，annotationProcessor，compileOnly等。


|语法|旧语法|功能|代码隔离效果|
|:---:|:---:|:---:|:---:|
|implementation|compile|编译期间对自己组件可见，对其他组件不可见，运行期间可见|编译器隔离|
|api|compile|编译期和运行期间都可见|没有隔离|
|compileOnly|provided|只参与到编译期间，不打包到Apk里（也就是写代码时能引入包，但是运行时没有）|没有隔离|
|runtimeOnly|apk|编译期间不可见，但是会打包到Apk里，运行期可见|编译期间隔离|


提示：在 Android Studio 的最新版本中，默认没有
implementation fileTree(include: ['*.jar', '*.aar'], dir: 'libs')
因为如果项目使用到 jar 包，aar 包很容易造成项目之间的资源冲突。 所以官方更加推荐通过 implementation 进行导入。

## defaultConfig

```
compileSdkVersion 27                      // 设置编译时的 Android 版本
defaultConfig {
  applicationId "com.dicky.myapplication" // 项目的包名
...
buildTypes { // 环境配置
  release {  // 发行版环境
    buildConfigField("boolean", "LOG_DEBUG", "false")     // 配置日志
    buildConfigField("String", "URL_PERFIX", ""https://release.cn/"") // 配置前缀
    minifyEnabled false // 是否对代码进行混淆
    proguardFiles getDefaultProguardFile('proguard-android.txt'), 'proguard-rules.pro' // 指定混淆的规则文件
    signingConfig signingConfigs.release // 设置签名信息
    zipAlignEnabled true // 是否对执行 ZIP 对齐优化，减小 zip 体积，增加运行效率
  }
  debug {    // 测试版环境
    buildConfigField("boolean", "LOG_DEBUG", "true") // 配置日志
    buildConfigField("String", "URL_PERFIX", ""https://test.com/"")  // 配置前缀
    minifyEnabled false // 是否对代码进行混淆
    proguardFiles getDefaultProguardFile('proguard-android.txt'), 'proguard-rules.pro' // 指定混淆的规则文件
    signingConfig signingConfigs.debug // 设置签名信息
    debuggable false // 是否支持断点调试
    jniDebuggable false // 是否可以调试 NDK 代码
    renderscriptDebuggable false // 是否开启渲染脚本就是一些 c 写的渲染方法
    zipAlignEnabled true // 是否对APK包执行ZIP对齐优化，减小 zip 体积，增加运行效率
