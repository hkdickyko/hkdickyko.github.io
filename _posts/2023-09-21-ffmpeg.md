...
ffmpeg 中的 **-filter**/**-vf**/**-af** 和 **-filter_complex** 选项以及 ffplay 中的 **-vf**/**-af** 中函数识别。过滤器链由一系列连接的过滤器组成，每个过滤器都连接到序列中的前一个过滤器。 过滤器链由 <font color="#FF0010">**,**</font> 分隔的过滤器描述列表表示。
...
![ffmpeg](../assets/img/ffmpeg/ffmpegsyntax.png)
 ...
 - 参数列表可以使用字符 **'**  作为起始和结束标记，并使用字符 **\\** 来转义引用文本中的字符； 否则，当遇到下一个特殊字符（属于集合 **[]=;,** ）时，参数字符串被视为终止。
