...
function getFormDict() {
    ...
    outDict['subfunction'] = $('#subfunctionName')[0].value;
    valid = checkArgName(outDict['subfunction']);
    if (valid != null) {
        $("#codeOutput").text("ERROR: Sub-function name - " + valid);
        return null;
    }
...
function generateCode() {
    ...
    outputCode += 'STATIC mp_obj_t ' + formDict['subfunction'] + '(';
    ...
    switch (formDict['output']['type']) {
        ...
        case 'string':
            retType = '';
            retCode = INDENT + '// signature: mp_obj_t mp_obj_new_str(const char* data, size_t len); size_t = int;\n'
    ...
    outputCode += 'STATIC '
        // Function wrapper
    if (numInputs < 4) {
        outputCode += 'MP_DEFINE_CONST_FUN_OBJ_' + numInputs + '(' + formDict['subfunction'] + '_obj, ' + formDict['subfunction'] + ');\n';
    } else {
    ...
    // Map table
    outputCode += '\n';
    outputCode += 'STATIC const mp_rom_map_elem_t ' + formDict['function'] + '_module_globals_table[] = {\n';
    outputCode += INDENT + '{ MP_ROM_QSTR(MP_QSTR___name__), MP_ROM_QSTR(MP_QSTR_' + formDict['function'] + ') },\n';
    outputCode += INDENT + '\n';
    outputCode += INDENT + '// --- Item below is the function need to copy to core file ---\n';
    outputCode += INDENT + '{ MP_ROM_QSTR(MP_QSTR_' + formDict['subfunction'] + '), MP_ROM_PTR(&' + formDict['subfunction'] + '_obj) },\n'
    outputCode += INDENT + '// --- End ---------------------\n';
    outputCode += INDENT + '};';
    outputCode += '\nSTATIC MP_DEFINE_CONST_DICT(' + formDict['function'] + '_module_globals, ' + formDict['function'] + '_module_globals_table);\n';

    outputCode += '\nconst mp_obj_module_t ' + formDict['function'] + '_user_cmodule = {';
    outputCode += '\n\t.base = { &mp_type_module },';
    outputCode += '\n\t.globals = (mp_obj_dict_t*)&' + formDict['function'] + '_module_globals,';
    outputCode += '\n};';
    outputCode += '\nMP_REGISTER_MODULE(MP_QSTR_' + formDict['function'] + ', ' + formDict['function'] + '_user_cmodule, MODULE_' + formDict['function'].toUpperCase() + '_ENABLED);';

    outputCode += '\n\n// Copy below define in the file ~/Micropython/micropython/ports/esp32/mpconfigport.h';
    outputCode += '\n#define MODULE_' + formDict['function'].toUpperCase() + '_ENABLED (1)'
